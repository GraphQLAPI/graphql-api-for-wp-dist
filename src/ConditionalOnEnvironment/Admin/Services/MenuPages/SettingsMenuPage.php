        ...
        \add_action('admin_init', function () {
            ...
            foreach ($items as $item) {
                ...
                foreach ($item['settings'] as $itemSetting) {
                    \add_settings_field($itemSetting[Properties::NAME], $itemSetting[Properties::TITLE] ?? '', function () use ($module, $itemSetting) {
                    ...
                    }, self::SETTINGS_FIELD, $settingsFieldForModule, [
             ...
             */
            \register_setting(self::SETTINGS_FIELD, Options::SETTINGS, [
                'type' => 'array',
        ...
        >
            ...
            <?php if ($printWithTabs) : ?>
                ...
                <h2 class="nav-tab-wrapper">
                    <?php
                    foreach ($items as $item) {
                        printf('<a href="#%s" class="nav-tab %s">%s</a>', $item['id'], $item['id'] == $activeModuleID ? 'nav-tab-active' : '', $item['name']);
                    }
                    ?>
                </h2>
            <?php endif; ?>
            ...
            <form method="post" action="options.php">
                ...
                foreach ($items as $item) {
                    $sectionStyle = '';
                    $maybeTitle = $printWithTabs ? '' : sprintf('<hr/><h3>%s</h3>', $item['name']);
                    if ($printWithTabs) {
                        $sectionStyle = sprintf('display: %s;', $item['id'] == $activeModuleID ? 'block' : 'none');
                    }
                    ?>
