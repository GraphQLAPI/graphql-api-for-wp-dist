...
abstract class AbstractDocAboutMenuPage extends AbstractDocsMenuPage
{
    ...
    {
        return array_diff($specialChars, [
            '/',
        ]);
    }
    ...
    {
        // Enable "/" in the filename
        add_filter('sanitize_file_name_chars', [$this, 'enableSpecialCharsForSanitization']);
        $filename = $_REQUEST[RequestParams::DOC] ?? '';
        $doc = \sanitize_file_name($filename . '.md');
        remove_filter('sanitize_file_name_chars', [$this, 'enableSpecialCharsForSanitization']);
        ...
        try {
            ...
            return sprintf('<p>%s</p>', sprintf(\__('Page \'%s\' does not exist', 'graphql-api'), $doc));
        }
    }
}
