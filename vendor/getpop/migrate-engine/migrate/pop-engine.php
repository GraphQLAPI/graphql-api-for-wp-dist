...
class Plugin
{
    public function __construct()
    {
        ...
        \PoP\Hooks\Facades\HooksAPIFacade::getInstance()->addAction('plugins_loaded', array($this, 'init'), 88823);
    }
    public function init()
    {
        if ($this->validate()) {
            $this->initialize();
            \define('POP_ENGINE_INITIALIZED', \true);
            // Allow plug-ins to override values
            \PoP\Hooks\Facades\HooksAPIFacade::getInstance()->addAction('plugins_loaded', array($this, 'defineConstants'), 110);
        }
    }
    ...
    public function validate()
    {
        ...
        $validation = new \PoP\Engine\Validation();
        return $validation->validate();
    }
    public function initialize()
    {
        include_once 'initialization.php';
        $initialization = new \PoP\Engine\Initialization();
