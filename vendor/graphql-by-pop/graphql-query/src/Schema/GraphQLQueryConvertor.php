...
class GraphQLQueryConvertor implements GraphQLQueryConvertorInterface
{
    ...
    public function convertFromGraphQLToFieldQuery(
        ...
        ) = $this->convertFromGraphQLToFieldQueryPaths($graphQLQuery, $variables ?? [], $enableMultipleQueryExecution, $operationName);
        $fieldQueries = [];
        ...
        foreach (array_reverse($fieldDirectives) as $directive) {
            ...
            if ($enableComposableDirectives) {
                ...
                if (isset($composableDirectivesByPosition[$counter])) {
                    $directiveComposableDirectives = QuerySyntax::SYMBOL_FIELDDIRECTIVE_OPENING . implode(QuerySyntax::SYMBOL_FIELDDIRECTIVE_SEPARATOR, array_map([$fieldQueryInterpreter, 'convertDirectiveToFieldDirective'], $composableDirectivesByPosition[$counter])) . QuerySyntax::SYMBOL_FIELDDIRECTIVE_CLOSING;
                }
            }
            ...
            if ($enableComposableDirectives && $nestedUnder !== null) {
                if (!is_int($nestedUnder) || !($nestedUnder < 0)) {
                    $this->feedbackMessageStore->addQueryError(sprintf($this->translationAPI->__('Param \'%s\' must be a negative integer, hence value \'%s\' in directive \'%s\' has been ignored', 'graphql-query'), SchemaElements::DIRECTIVE_PARAM_NESTED_UNDER, $nestedUnder, $directiveName));
                } elseif ((-1 * $nestedUnder) > $counter) {
                    $this->feedbackMessageStore->addQueryError(sprintf($this->translationAPI->__('There is no directive at position \'%s\' (set under param \'%s\') relative to directive \'%s\'', 'graphql-query'), $nestedUnder, SchemaElements::DIRECTIVE_PARAM_NESTED_UNDER, $directiveName));
                } else {
    ...
    protected function restrainFieldsByTypeOrInterface(array $fragmentFieldPaths, string $fragmentModel): array
    {
        ...
        $includeDirective = $fieldQueryInterpreter->composeFieldDirective(IncludeDirectiveResolver::getDirectiveName(), $fieldQueryInterpreter->getFieldArgsAsString([
            'if' => $fieldQueryInterpreter->getField('or', [
                'values' => [
                    $fieldQueryInterpreter->getField('isType', [
                        'type' => $fragmentModel
                    ]),
                    $fieldQueryInterpreter->getField('implements', [
