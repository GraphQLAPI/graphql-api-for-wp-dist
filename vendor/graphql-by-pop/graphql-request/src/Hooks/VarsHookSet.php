...
{
    /**
     * @var \GraphQLByPoP\GraphQLRequest\Execution\QueryRetrieverInterface
     */
    protected $queryRetrieverInterface;
    public function __construct(HooksAPIInterface $hooksAPI, TranslationAPIInterface $translationAPI, InstanceManagerInterface $instanceManager, QueryRetrieverInterface $queryRetrieverInterface)
    {
    ...
    public function getMutationsNotSupportedErrorMessage(string $errorMessage) : string
    {
        ...
        if ($vars['standard-graphql']) {
            return \sprintf($this->translationAPI->__('Use the operation type \'%s\' to execute mutations', 'graphql-request'), OperationTypes::MUTATION);
        }
        return $errorMessage;
    }
    /**
    ...
    protected function processURLParamVars(array &$vars) : void
    {
        ...
        if (!isset($_REQUEST[QueryInputs::QUERY]) || ComponentConfiguration::disableGraphQLAPIForPoP() || $isGraphQLPersistedQuery) {
            // Add a flag indicating that we are doing standard GraphQL
            ...
            if ($isGraphQLPersistedQuery) {
                ...
                list($graphQLQuery, $variables, $operationName) = $this->queryRetrieverInterface->extractRequestedGraphQLQueryPayload();
            }
            ...
            if ($graphQLQuery) {
                ...
                $errorMessage = $disablePoPQuery ? $this->translationAPI->__('No query was provided. (The body has no query, and the query provided as a URL param is ignored because of configuration)', 'graphql-request') : $this->translationAPI->__('The query in the body is empty', 'graphql-request');
    ...
    public function addGraphQLQueryToVars(array &$vars, string $graphQLQuery, ?string $operationName = null) : void
    {
        ...
        list($operationType, $fieldQuery) = $graphQLQueryConvertor->convertFromGraphQLToFieldQuery($graphQLQuery, $variables, ComponentConfiguration::enableMultipleQueryExecution(), $operationName);
