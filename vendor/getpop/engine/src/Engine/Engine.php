...
class Engine extends \PoP\ComponentModel\Engine\Engine implements \PoP\Engine\Engine\EngineInterface
{
    /**
    ...
    /**
     * @var \PoP\CacheControl\Managers\CacheControlEngineInterface
     */
    protected $cacheControlEngine;
    public function __construct(TranslationAPIInterface $translationAPI, HooksAPIInterface $hooksAPI, DataStructureManagerInterface $dataStructureManager, InstanceManagerInterface $instanceManager, ModelInstanceInterface $modelInstance, FeedbackMessageStoreInterface $feedbackMessageStore, ModulePathHelpersInterface $modulePathHelpers, ModulePathManagerInterface $modulePathManager, FieldQueryInterpreterInterface $fieldQueryInterpreter, ModuleFilterManagerInterface $moduleFilterManager, ModuleProcessorManagerInterface $moduleProcessorManager, CheckpointProcessorManagerInterface $checkpointProcessorManager, DataloadHelperServiceInterface $dataloadHelperService, EntryModuleManagerInterface $entryModuleManager, RequestHelperServiceInterface $requestHelperService, LooseContractManagerInterface $looseContractManager, CacheControlEngineInterface $cacheControlEngine, ?CacheInterface $persistentCache = null)
    {
        $this->looseContractManager = $looseContractManager;
        $this->cacheControlEngine = $cacheControlEngine;
        parent::__construct($translationAPI, $hooksAPI, $dataStructureManager, $instanceManager, $modelInstance, $feedbackMessageStore, $modulePathHelpers, $modulePathManager, $fieldQueryInterpreter, $moduleFilterManager, $moduleProcessorManager, $checkpointProcessorManager, $dataloadHelperService, $entryModuleManager, $requestHelperService, $persistentCache);
    }
    public function generateData() : void
    {
        ...
        if ($notImplementedHooks = $this->looseContractManager->getNotImplementedRequiredHooks()) {
            throw new Exception(\sprintf($this->translationAPI->__('The following hooks have not been implemented by the CMS: "%s". Hence, we can\'t continue.'), \implode($this->translationAPI->__('", "'), $notImplementedHooks)));
        }
        if ($notImplementedNames = $this->looseContractManager->getNotImplementedRequiredNames()) {
            throw new Exception(\sprintf($this->translationAPI->__('The following names have not been implemented by the CMS: "%s". Hence, we can\'t continue.'), \implode($this->translationAPI->__('", "'), $notImplementedNames)));
        }
        parent::generateData();
    }
