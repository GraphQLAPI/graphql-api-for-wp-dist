    ...
    public function resolveDirective(
        ...
        if ($idsDataFields) {
            ...
            foreach ($idsDataFields as $id => $dataFields) {
                $fields = array_merge($fields, $dataFields['direct']);
            }
            $fields = array_values(array_unique($fields));
            // Extract all the field arguments which are fields or have fields themselves
            $fieldArgElems = array_unique(GeneralUtils::arrayFlatten(array_map(function ($field) use ($fieldQueryInterpreter) {
                if ($fieldArgs = $fieldQueryInterpreter->getFieldArgs($field)) {
            ...
            $nestedFields = array_unique(GeneralUtils::arrayFlatten((array)$fieldQueryInterpreter->maybeConvertFieldArgumentArrayValue($fieldArgElems), true));
