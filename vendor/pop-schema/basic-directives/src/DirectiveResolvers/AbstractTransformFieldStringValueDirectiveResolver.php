    ...
    protected function validateTypeIsString($value, $id, string $field, string $fieldOutputKey, array &$dbErrors, array &$dbWarnings)
    {
        if (!is_string($value)) {
            ...
            $dbWarnings[(string)$id][] = [
                ...
                Tokens::MESSAGE => sprintf($translationAPI->__('Directive \'%s\' from field \'%s\' cannot be applied on object with ID \'%s\' because it is not a string', 'practical-directives'), $this->getDirectiveName(), $fieldOutputKey, $id),
            ];
            return false;
        }
        return true;
