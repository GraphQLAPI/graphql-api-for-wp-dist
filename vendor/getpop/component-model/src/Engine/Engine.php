...
class Engine implements \PoP\ComponentModel\Engine\EngineInterface
{
    ...
    /**
     * @param \PoP\ComponentModel\DataStructure\DataStructureManagerInterface $dataStructureManager
     */
    public final function setDataStructureManager($dataStructureManager) : void
    {
    ...
    protected final function getDataStructureManager() : DataStructureManagerInterface
    {
        ...
        return $this->dataStructureManager = $this->dataStructureManager ?? $this->instanceManager->getInstance(DataStructureManagerInterface::class);
    }
    /**
    ...
    /**
     * @param \PoP\ComponentModel\EntryComponent\EntryComponentManagerInterface $entryComponentManager
     */
    public final function setEntryComponentManager($entryComponentManager) : void
    {
    ...
    protected final function getEntryComponentManager() : EntryComponentManagerInterface
    {
        ...
        return $this->entryComponentManager = $this->entryComponentManager ?? $this->instanceManager->getInstance(EntryComponentManagerInterface::class);
    }
    /**
    ...
    /**
     * @param \PoP\ComponentModel\Feedback\FeedbackEntryManagerInterface $feedbackEntryService
     */
    public final function setFeedbackEntryManager($feedbackEntryService) : void
    {
    ...
    protected final function getFeedbackEntryManager() : FeedbackEntryManagerInterface
    {
        ...
        return $this->feedbackEntryService = $this->feedbackEntryService ?? $this->instanceManager->getInstance(FeedbackEntryManagerInterface::class);
    }
    /**
     ...
     */
    protected function getEncodedDataObject($data) : array
    {
    ...
    protected function addSharedMeta() : void
    {
        ...
        if (\in_array(DataOutputItems::META, $dataoutputitems)) {
            ...
            if ($this->getExtraRoutes()) {
                $engineState->data['requestmeta'][Response::MULTIPLE_ROUTES] = \true;
            }
    ...
    public function getRequestMeta() : array
    {
        ...
        $meta = array(Response::ENTRY_COMPONENT => $entryComponent->name, Response::UNIQUE_ID => $moduleInfo->getUniqueID(), 'modelinstanceid' => $this->getModelInstance()->getModelInstanceID());
        if (App::isHTTPRequest()) {
     ...
     * @param \PoP\ComponentModel\Component\Component $component
     */
    protected function getInterreferencedComponentFullPaths($component, &$props) : array
    {
        $paths = [];
     ...
     * @param \PoP\ComponentModel\Component\Component $component
     */
    protected function getDataloadingComponentFullPaths($component, &$props) : array
    {
        $paths = [];
     ...
     * @param string $key
     */
    protected function assignValueForComponent(&$array, $component_path, $component, $key, $value) : void
    {
        /** @var ModuleInfo */
        $moduleInfo = App::getModule(Module::class)->getInfo();
     ...
     * @param \PoP\ComponentModel\Component\Component $component
     */
    protected function getComponentPathKey($component_path, $component) : string
    {
        $componentFullName = $this->getComponentHelpers()->getComponentFullName($component);
    ...
    public function getComponentData($root_component, $root_model_props, $root_props) : array
    {
        ...
        App::doAction(self::HOOK_ENGINE_ITERATION_START, $root_component, array(&$root_model_props), array(&$root_props), array(&$engineState->helperCalculations), $this);
        // First check if there's a cache stored
        ...
        foreach ($component_fullpaths as $component_path) {
            ...
            // Allow PoP UserState to add the lazy-loaded userstate data triggers
            App::doAction(self::HOOK_ENGINE_ITERATION_ON_DATALOADING_COMPONENT, $component, array(&$component_props), array(&$data_properties), $dataaccess_checkpoint_validation, $mutation_checkpoint_validation, $executed, $objectIDOrIDs, array(&$engineState->helperCalculations), $this);
        ...
        // Allow PoP UserState to add the lazy-loaded userstate data triggers
        App::doAction(self::HOOK_ENGINE_ITERATION_END, $root_component, array(&$root_model_props), array(&$root_props), array(&$engineState->helperCalculations), $this);
        return $ret;
    }
    /**
     * @return array<string,mixed>
     ...
     */
    protected function generateDatabases(&$schemaFeedbackEntries, &$objectFeedbackEntries) : array
    {
        $engineState = App::getEngineState();
     ...
     * @param \PoP\ComponentModel\TypeResolvers\ObjectType\ObjectTypeResolverInterface $targetObjectTypeResolver
     * @param string $component_path_key
     */
    protected function processSubcomponentData($relationalTypeResolver, $targetObjectTypeResolver, $typeResolverIDs, $component_path_key, &$databases, $subcomponents_data_properties, &$already_loaded_id_fields, &$unionTypeOutputKeyIDs, &$combinedUnionTypeOutputKeyIDs) : void
    {
     ...
     * @param \PoP\Root\Feedback\FeedbackItemResolution|null $mutation_checkpoint_validation
     */
    protected function processAndAddComponentData($component_path, $component, &$props, $data_properties, $dataaccess_checkpoint_validation, $mutation_checkpoint_validation, $executed, $objectIDs) : void
    {
        $processor = $this->getComponentProcessorManager()->getComponentProcessor($component);
        $engineState = App::getEngineState();
        // Integrate the feedback into $componentdata
