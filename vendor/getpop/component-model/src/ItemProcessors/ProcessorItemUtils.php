...
class ProcessorItemUtils
{
    public static function getItemFullName(array $item) : string
    {
        ...
        $itemFullName = $item[0] . \PoP\ComponentModel\ItemProcessors\Constants::SEPARATOR_PROCESSORITEMFULLNAME . $item[1];
        if (\count($item) >= 3) {
            $itemFullName .= \PoP\ComponentModel\ItemProcessors\Constants::SEPARATOR_PROCESSORITEMFULLNAME . \serialize($item[2]);
        }
        return $itemFullName;
    }
    public static function getItemFromFullName(string $itemFullName) : ?array
    {
        $parts = \explode(\PoP\ComponentModel\ItemProcessors\Constants::SEPARATOR_PROCESSORITEMFULLNAME, $itemFullName);
        ...
        return \count($parts) >= 3 ? [
            ...
            \unserialize(
                ...
                \substr($itemFullName, \strlen($parts[0] . \PoP\ComponentModel\ItemProcessors\Constants::SEPARATOR_PROCESSORITEMFULLNAME . $parts[1] . \PoP\ComponentModel\ItemProcessors\Constants::SEPARATOR_PROCESSORITEMFULLNAME))
            ),
        ] : $parts;
    }
