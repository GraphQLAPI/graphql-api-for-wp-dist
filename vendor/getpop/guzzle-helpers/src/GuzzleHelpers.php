...
class GuzzleHelpers
{
    ...
    public static function requestJSON(string $url, array $bodyJSONQuery = [], string $method = 'POST')
    {
        ...
        try {
            ...
            return new Error('request-failed', $exception->getMessage());
        }
    }
    /**
     * @return mixed
    ...
    protected static function validateAndDecodeJSONResponse(ResponseInterface $response)
    {
        ...
        if ($response->getStatusCode() != 200) {
            // Throw an error
            return new Error('request-failed', \sprintf($translationAPI->__('The response status code is \'%s\' instead of the expected \'%s\'', 'guzzle-helpers'), $response->getStatusCode(), 200));
        }
        ...
        if (!$isJSONContentType) {
            // Throw an error
            return new Error('request-failed', \sprintf($translationAPI->__('The response content type \'%s\' is unsupported', 'guzzle-helpers'), $contentType));
        }
        $bodyResponse = $response->getBody()->__toString();
        if (!$bodyResponse) {
            // Throw an error
            return new Error('request-failed', $translationAPI->__('The body of the response is empty', 'guzzle-helpers'));
        }
        return \json_decode($bodyResponse, \true);
    }
    /**
