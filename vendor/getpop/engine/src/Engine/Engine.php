...
class Engine extends \PoP\ComponentModel\Engine\Engine implements EngineInterface
{
    public function generateData()
    {
        ...
        if ($notImplementedHooks = $looseContractManager->getNotImplementedRequiredHooks()) {
            throw new Exception(
                sprintf($translationAPI->__('The following hooks have not been implemented by the CMS: "%s". Hence, we can\'t continue.'), implode($translationAPI->__('", "'), $notImplementedHooks))
            );
        }
        if ($notImplementedNames = $looseContractManager->getNotImplementedRequiredNames()) {
            throw new Exception(
                sprintf($translationAPI->__('The following names have not been implemented by the CMS: "%s". Hence, we can\'t continue.'), implode($translationAPI->__('", "'), $notImplementedNames))
            );
        }
