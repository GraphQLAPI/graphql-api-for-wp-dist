...
class Engine_Utils
{
    public static function getCustomPostPath($post_id, $remove_post_slug = \false)
    {
        ...
        $domain = \PoP\ComponentModel\Misc\GeneralUtils::maybeAddTrailingSlash($cmsengineapi->getHomeURL());
        ...
        if ($remove_post_slug) {
            ...
            $post_path = \substr($post_path, 0, \strlen($post_path) - \strlen(\PoP\ComponentModel\Misc\GeneralUtils::maybeAddTrailingSlash($post_slug)));
        }
        return $post_path;
    }
}
