    ...
    public function getSchemaDirectiveArgs(TypeResolverInterface $typeResolver): array
    {
        ...
        return [
            ...
            [
                ...
                SchemaDefinition::ARGNAME_DESCRIPTION => sprintf($translationAPI->__('Arguments to inject to the function. The value of the affected field can be provided under special expression `%s`', 'component-model'), QueryHelpers::getExpressionQuery(Expressions::NAME_VALUE)),
            ],
            [
        ...
        foreach ($idsDataFields as $id => $dataFields) {
            foreach ($dataFields['direct'] as $field) {
                ...
                if ($schemaDBWarnings) {
                    foreach ($schemaDBWarnings as $warningMessage) {
                        $dbWarnings[(string)$id][] = [
                            Tokens::PATH => [$this->directive],
                            Tokens::MESSAGE => sprintf($translationAPI->__('Warning validating function \'%s\' on object with ID \'%s\' and field under property \'%s\': %s)', 'component-model'), $function, $id, $fieldOutputKey, $warningMessage),
                        ];
                    }
                }
                if ($schemaDBErrors) {
                    foreach ($schemaDBErrors as $errorMessage) {
                        $dbErrors[(string)$id][] = [
                            Tokens::PATH => [$this->directive],
                            Tokens::MESSAGE => sprintf($translationAPI->__('Error validating function \'%s\' on object with ID \'%s\' and field under property \'%s\': %s)', 'component-model'), $function, $id, $fieldOutputKey, $errorMessage),
                        ];
                    }
                    if ($fieldOutputKey != $field) {
                        $dbErrors[(string)$id][] = [
                            Tokens::PATH => [$this->directive],
                            Tokens::MESSAGE => sprintf($translationAPI->__('Applying function on field \'%s\' (under property \'%s\') on object with ID \'%s\' can\'t be executed due to previous errors', 'component-model'), $field, $fieldOutputKey, $id),
                        ];
                    } else {
                        $dbErrors[(string)$id][] = [
                            Tokens::PATH => [$this->directive],
                            Tokens::MESSAGE => sprintf($translationAPI->__('Applying function on field \'%s\' on object with ID \'%s\' can\'t be executed due to previous errors', 'component-model'), $fieldOutputKey, $id),
                        ];
                    }
                    continue;
                }
                ...
                if (GeneralUtils::isError($functionValue)) {
                    ...
                    $dbErrors[(string)$id][] = [
                        Tokens::PATH => [$this->directive],
                        Tokens::MESSAGE => sprintf($translationAPI->__('Applying function on \'%s\' on object with ID \'%s\' failed due to error: %s', 'component-model'), $fieldOutputKey, $id, $error->getErrorMessage()),
                    ];
                    continue;
                }
