...
class SchemaHelpers
{
    ...
    {
        return \array_values(\array_filter($fieldArgProps, function ($fieldArgProp) use($fieldArgs) {
            return !\array_key_exists($fieldArgProp, $fieldArgs);
        }));
    }
    public static function getSchemaMandatoryFieldArgs(array $schemaFieldArgs)
    {
        return \array_filter($schemaFieldArgs, function ($schemaFieldArg) {
            return isset($schemaFieldArg[\PoP\ComponentModel\Schema\SchemaDefinition::ARGNAME_MANDATORY]) && $schemaFieldArg[\PoP\ComponentModel\Schema\SchemaDefinition::ARGNAME_MANDATORY];
        });
    }
    public static function getSchemaEnumTypeFieldArgs(array $schemaFieldArgs)
    {
        return \array_filter($schemaFieldArgs, function ($schemaFieldArg) {
            return isset($schemaFieldArg[\PoP\ComponentModel\Schema\SchemaDefinition::ARGNAME_TYPE]) && $schemaFieldArg[\PoP\ComponentModel\Schema\SchemaDefinition::ARGNAME_TYPE] == \PoP\ComponentModel\Schema\SchemaDefinition::TYPE_ENUM;
        });
    }
    public static function getSchemaFieldArgNames(array $schemaFieldArgs)
    {
        ...
        return \array_values(\array_map(function ($schemaFieldArg) {
            return $schemaFieldArg[\PoP\ComponentModel\Schema\SchemaDefinition::ARGNAME_NAME];
        }, $schemaFieldArgs));
    }
    public static function convertToSchemaFieldArgEnumValueDefinitions(array $enumValues)
    {
        ...
        foreach ($enumValues as $enumValue) {
            $enumValueDefinitions[$enumValue] = [\PoP\ComponentModel\Schema\SchemaDefinition::ARGNAME_NAME => $enumValue];
        }
    ...
    public static function removeDeprecatedEnumValuesFromSchemaDefinition(array $enumValueDefinitions) : array
    {
        ...
        return \array_filter($enumValueDefinitions, function ($enumValueDefinition) {
            if ($enumValueDefinition[\PoP\ComponentModel\Schema\SchemaDefinition::ARGNAME_DEPRECATED] ?? null) {
                return \false;
            }
            return \true;
        });
    ...
    {
        $enumValuesOrDefinitions = \array_map(function ($schemaFieldArg) {
            return $schemaFieldArg[\PoP\ComponentModel\Schema\SchemaDefinition::ARGNAME_ENUM_VALUES];
        }, $schemaFieldArgs);
        ...
        foreach ($enumValuesOrDefinitions as $fieldArgName => $fieldArgEnumValuesOrDefinitions) {
            ...
            if (\is_array($fieldArgEnumValuesOrDefinitions[$firstElemKey])) {
                ...
                foreach ($fieldArgEnumValuesOrDefinitions as $enumValue) {
                    $enumValueDefinitions[$fieldArgName][$enumValue] = [\PoP\ComponentModel\Schema\SchemaDefinition::ARGNAME_NAME => $enumValue];
                }
            }
        }
        return $enumValueDefinitions;
    ...
    public static function getTypeComponents(string $type) : array
    {
        ...
        $arrayInstances = 0;
        while ($convertedType && \PoP\ComponentModel\Schema\TypeCastingHelpers::getTypeCombinationCurrentElement($convertedType) == \PoP\ComponentModel\Schema\SchemaDefinition::TYPE_ARRAY) {
            $arrayInstances++;
            $convertedType = \PoP\ComponentModel\Schema\TypeCastingHelpers::getTypeCombinationNestedElements($convertedType);
        }
        ...
        if (!$convertedType) {
            ...
            throw new \InvalidArgumentException(\sprintf($translationAPI->__('Type \'%s\' doesn\'t declare the type of the innermost element'), $type));
        }
        return [$arrayInstances, $convertedType];
    }
    /**
     ...
     * @return string
     */
    public static function convertTypeIDToTypeName(string $type, \PoP\ComponentModel\TypeResolvers\TypeResolverInterface $typeResolver, string $fieldName) : string
    {
        list($arrayInstances, $convertedType) = self::getTypeComponents($type);
        // If the type is an ID, replace it with the actual type the ID references
        if ($convertedType == \PoP\ComponentModel\Schema\SchemaDefinition::TYPE_ID) {
        ...
        return \PoP\ComponentModel\Schema\TypeCastingHelpers::makeArray($convertedType, $arrayInstances);
    }
    public static function getSchemaNamespace(string $namespace) : string
    {
        return self::convertNamespace(self::getOwnerAndProjectFromNamespace($namespace));
    }
    /**
    ...
    {
        return \str_replace('\\', \PoP\ComponentModel\Schema\SchemaDefinition::TOKEN_NAMESPACE_SEPARATOR, $namespace);
    }
    public static function getSchemaNamespacedName(string $schemaNamespace, string $name) : string
    {
        return ($schemaNamespace ? $schemaNamespace . \PoP\ComponentModel\Schema\SchemaDefinition::TOKEN_NAMESPACE_SEPARATOR : '') . $name;
    }
}
