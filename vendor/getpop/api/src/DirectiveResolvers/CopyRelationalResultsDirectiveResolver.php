...
class CopyRelationalResultsDirectiveResolver extends AbstractGlobalDirectiveResolver
{
    ...
    {
        return [[SchemaDefinition::ARGNAME_NAME => 'copyFromFields', SchemaDefinition::ARGNAME_TYPE => SchemaDefinition::TYPE_STRING, SchemaDefinition::ARGNAME_IS_ARRAY => \true, SchemaDefinition::ARGNAME_DESCRIPTION => $this->translationAPI->__('The fields in the relational object from which to copy the data', 'component-model'), SchemaDefinition::ARGNAME_MANDATORY => \true], [SchemaDefinition::ARGNAME_NAME => 'copyToFields', SchemaDefinition::ARGNAME_TYPE => SchemaDefinition::TYPE_STRING, SchemaDefinition::ARGNAME_IS_ARRAY => \true, SchemaDefinition::ARGNAME_DESCRIPTION => $this->translationAPI->__('The fields in the current object to which copy the data. Default value: Same fields provided through \'copyFromFields\' argument', 'component-model')], [SchemaDefinition::ARGNAME_NAME => 'keepRelationalIDs', SchemaDefinition::ARGNAME_TYPE => SchemaDefinition::TYPE_BOOL, SchemaDefinition::ARGNAME_DESCRIPTION => $this->translationAPI->__('Indicate if the properties are placed under the relational ID as keys (`true`) or as a one-dimensional array (`false`)', 'component-model'), SchemaDefinition::ARGNAME_DEFAULT_VALUE => \false]];
    }
    /**
    ...
    public function validateDirectiveArgumentsForSchema(TypeResolverInterface $typeResolver, string $directiveName, array $directiveArgs, array &$schemaErrors, array &$schemaWarnings, array &$schemaDeprecations) : array
    {
        ...
        if (isset($directiveArgs['copyToFields'])) {
            ...
            if ($copyToFieldsCount > $copyFromFieldsCount) {
                $schemaWarnings[] = [Tokens::PATH => [$this->directive], Tokens::MESSAGE => \sprintf($this->translationAPI->__('Argument \'copyToFields\' has more elements than argument \'copyFromFields\', so the following fields have been ignored: \'%s\'', 'component-model'), \implode($this->translationAPI->__('\', \''), \array_slice($copyToFields, $copyFromFieldsCount)))];
            } elseif ($copyToFieldsCount < $copyFromFieldsCount) {
                $schemaWarnings[] = [Tokens::PATH => [$this->directive], Tokens::MESSAGE => \sprintf($this->translationAPI->__('Argument \'copyFromFields\' has more elements than argument \'copyToFields\', so the following fields will be copied to the destination object under their same field name: \'%s\'', 'component-model'), \implode($this->translationAPI->__('\', \''), \array_slice($copyFromFields, $copyToFieldsCount)))];
            }
        }
                ...
                foreach ($dataFields['direct'] as $relationalField) {
                    ...
                    if (!\array_key_exists($relationalFieldOutputKey, $previousDBItems[$dbKey][(string) $id] ?? [])) {
                        if ($relationalFieldOutputKey != $relationalField) {
                            $dbErrors[(string) $id][] = [Tokens::PATH => [$this->directive], Tokens::MESSAGE => \sprintf($this->translationAPI->__('Field \'%s\' (under property \'%s\') hadn\'t been set for object with ID \'%s\', so no data can be copied', 'component-model'), $relationalField, $relationalFieldOutputKey, $id)];
                        } else {
                            $dbErrors[(string) $id][] = [Tokens::PATH => [$this->directive], Tokens::MESSAGE => \sprintf($this->translationAPI->__('Field \'%s\' hadn\'t been set for object with ID \'%s\', so no data can be copied', 'component-model'), $relationalField, $id)];
                        }
                        continue;
                    }
                    ...
                    if ($isTargetValueInDBItems || \array_key_exists($copyToField, $previousDBItems[$dbKey][(string) $id] ?? [])) {
                        $dbWarnings[(string) $id][] = [Tokens::PATH => [$this->directive], Tokens::MESSAGE => \sprintf($this->translationAPI->__('The existing value for field \'%s\' from object with ID \'%s\' has been overriden: \'%s\'', 'component-model'), $copyToField, $id, $isTargetValueInDBItems ? $dbItems[(string) $id][$copyToField] : $previousDBItems[$dbKey][(string) $id][$copyToField])];
                    }
                    ...
                    foreach ($relationalIDs as $relationalID) {
                        ...
                        if (!\array_key_exists($copyFromField, $previousDBItems[$relationalDBKey][(string) $relationalID] ?? [])) {
                            $dbErrors[(string) $id][] = [Tokens::PATH => [$this->directive], Tokens::MESSAGE => \sprintf($this->translationAPI->__('Field \'%s\' hadn\'t been set for object of entity \'%s\' and ID \'%s\', so no data can be copied', 'component-model'), $copyFromField, $relationalDBKey, $relationalID)];
                            continue;
                        }
                        if ($copyStraight) {
