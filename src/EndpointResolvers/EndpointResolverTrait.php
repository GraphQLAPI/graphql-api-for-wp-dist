...
trait EndpointResolverTrait
{
    ...
    protected function executeGraphQLQuery(): void
    {
         ...
         */
        \add_action('ApplicationState:addVars', [$this, 'addGraphQLVars'], 1, 1);
        /**
         ...
         */
        \add_filter('WPCMSRoutingState:nature', [$this, 'getNature'], PHP_INT_MAX, 2);
    }
    ...
    public function addGraphQLVars(array $vars_in_array): void
    {
        ...
        if ($graphQLVariables) {
            ...
            $vars['variables'] = $this->doURLParamsOverrideGraphQLVariables($vars['routing-state']['queried-object-id']) ?
                array_merge($graphQLVariables, $vars['variables'] ?? []) :
                array_merge($vars['variables'] ?? [], $graphQLVariables);
        }
