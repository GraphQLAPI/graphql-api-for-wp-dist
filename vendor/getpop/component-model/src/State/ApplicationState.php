...
class ApplicationState
{
    ...
    public static function getVars() : array
    {
        ...
        $actions = isset($_REQUEST[Params::ACTIONS]) ? \array_map('strtolower', $_REQUEST[Params::ACTIONS]) : [];
        ...
        $version = Environment::enableVersionByParams() ? $_REQUEST[Params::VERSION] ?? $appVersion : $appVersion;
        $outputs = (array) HooksAPIFacade::getInstance()->applyFilters('ApplicationState:outputs', array(Outputs::HTML, Outputs::JSON));
        if (!\in_array($output, $outputs)) {
            $output = Outputs::HTML;
        }
        // Target/Module default values (for either empty, or if the user is playing around with the url)
        $alldatasources = array(DataSourceSelectors::ONLYMODEL, DataSourceSelectors::MODELANDREQUEST);
        if (!\in_array($datasources, $alldatasources)) {
            $datasources = DataSourceSelectors::MODELANDREQUEST;
        }
        $dataoutputmodes = array(DataOutputModes::SPLITBYSOURCES, DataOutputModes::COMBINED);
        ...
        $alldataoutputitems = (array) HooksAPIFacade::getInstance()->applyFilters('ApplicationState:dataoutputitems', array(DataOutputItems::META, DataOutputItems::DATASET_MODULE_SETTINGS, DataOutputItems::MODULE_DATA, DataOutputItems::DATABASES, DataOutputItems::SESSION));
        $dataoutputitems = \array_intersect($dataoutputitems ?? array(), $alldataoutputitems);
        if (!$dataoutputitems) {
            $dataoutputitems = HooksAPIFacade::getInstance()->applyFilters('ApplicationState:default-dataoutputitems', array(DataOutputItems::META, DataOutputItems::DATASET_MODULE_SETTINGS, DataOutputItems::MODULE_DATA, DataOutputItems::DATABASES, DataOutputItems::SESSION));
        }
        ...
        $targets = (array) HooksAPIFacade::getInstance()->applyFilters('ApplicationState:targets', array(Targets::MAIN));
        ...
        HooksAPIFacade::getInstance()->doAction('ApplicationState:addVars', array(&self::$vars));
    ...
    public static function augmentVarsProperties()
    {
        ...
        HooksAPIFacade::getInstance()->doAction('augmentVarsProperties', array(&self::$vars));
    }
}
