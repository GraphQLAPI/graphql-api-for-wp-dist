    ...
    public function maybeProcessAction()
    {
        ...
        if ($isSingleAction) {
            ...
            if (!\wp_verify_nonce($nonce, 'graphql_api_enable_or_disable_module')) {
                $noParamsCurrentURL = \admin_url(sprintf('admin.php?page=%s', $_GET['page'] ?? ''));
                \wp_die(sprintf(__('This URL is either stale or not valid. Please <a href="%s">click here to reload the page</a>, and try again', 'graphql-api'), $noParamsCurrentURL));
            }
