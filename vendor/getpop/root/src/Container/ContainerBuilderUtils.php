...
class ContainerBuilderUtils
{
     ...
     */
    public static function getServiceClassesUnderNamespace(string $namespace, bool $includeSubfolders = \true) : array
    {
        ...
        return \array_filter($containerBuilder->getServiceIds(), function ($class) use($namespace, $includeSubfolders) {
            return \strpos($class, $namespace) === 0 && ($includeSubfolders || \strpos($class, '\\', \strlen($namespace)) === \false);
        });
    }
    /**
     ...
     * @return void
     */
    public static function injectServicesIntoService(string $injectableServiceId, string $injectingServicesNamespace, string $methodCall, bool $includeSubfolders = \true) : void
    {
        /**
        ...
        foreach ($injectingServiceClasses as $injectingServiceClassId) {
            $definition->addMethodCall($methodCall, [new \PrefixedByPoP\Symfony\Component\DependencyInjection\Reference($injectingServiceClassId)]);
        }
    }
    /**
     ...
     * @return void
     */
    public static function injectServiceIntoService(string $injectableServiceId, string $injectingServiceId, string $methodCall) : void
    {
        /**
         * @var ContainerBuilder
         */
        $containerBuilder = \PoP\Root\Container\ContainerBuilderFactory::getInstance();
        $definition = $containerBuilder->getDefinition($injectableServiceId);
        $definition->addMethodCall($methodCall, [new \PrefixedByPoP\Symfony\Component\DependencyInjection\Reference($injectingServiceId)]);
    }
    /**
     ...
     * @return void
     */
    public static function injectValuesIntoService(string $injectableServiceId, string $methodCall, ...$values) : void
    {
        ...
        $values = \array_map(function ($value) {
            if ($value && \is_string($value) && \substr($value, 0, 1) == '@') {
                ...
                return new \PrefixedByPoP\Symfony\Component\DependencyInjection\Reference($injectedServiceId);
            }
            return $value;
        }, $values);
        /**
