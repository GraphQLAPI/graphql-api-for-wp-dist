...
class GD_FormInput_DateRangeTime extends \PoP\Engine\GD_FormInput_MultipleInputs
{
    public function __construct($params = array())
    {
        ...
        if (!isset($params['selected']) && $this->selected) {
            ...
            if ($from && $fromtime && $to && $totime) {
                ...
                $dateto = $to . ' ' . $totime . ':00';
                $this->selected['readable'] = \sprintf(\PoP\Translation\Facades\TranslationAPIFacade::getInstance()->__('%1$s%2$s%3$s', 'pop-application'), \date('d/m/Y g:i A', \strtotime($datefrom)), GD_DATERANGE_SEPARATOR, \date('d/m/Y g:i A', \strtotime($dateto)));
            }
        }
    }
}
