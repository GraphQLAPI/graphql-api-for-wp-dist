...
EOT;
            ...
            $maybeSchemaModeContent = sprintf($blockContentPlaceholder, \__('Public/Private Schema:', 'graphql-api'), isset($attributes[self::ATTRIBUTE_NAME_SCHEMA_MODE]) ?
                $schemaModeLabels[$attributes[self::ATTRIBUTE_NAME_SCHEMA_MODE]]
                : \__('Default', 'graphql-api'), $className . '__title', \__('Who can access:', 'graphql-api'));
        }
        if ($content) {
            return $maybeSchemaModeContent . $content;
        }
        return $maybeSchemaModeContent . sprintf('<em>%s</em>', \__('(not set)', 'graphql-api'));
    }
}
