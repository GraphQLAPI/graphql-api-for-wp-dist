    ...
    */
    ...
    protected function validateAndResolveInstances(
        ...
        for ($i = 0; $i < count($fieldDirectives); $i++) {
            ...
            if (is_null($fieldDirectiveResolverInstances)) {
                $schemaErrors[] = [
                    ...
                    Tokens::MESSAGE => sprintf($translationAPI->__('No DirectiveResolver resolves directive with name \'%s\'', 'pop-component-model'), $directiveName),
                ];
                if ($stopDirectivePipelineExecutionIfDirectiveFailed) {
                    $schemaErrors[] = [
                        Tokens::PATH => [$fieldDirective],
                        Tokens::MESSAGE => sprintf($stopDirectivePipelineExecutionPlaceholder, $fieldDirective),
                    ];
                    break;
                }
                continue;
            ...
            if (empty($fieldDirectiveResolverInstances)) {
                $schemaErrors[] = [
                    Tokens::PATH => [$fieldDirective],
                    Tokens::MESSAGE => sprintf($translationAPI->__('No DirectiveResolver processes directive with name \'%s\' and arguments \'%s\' in field(s) \'%s\'', 'pop-component-model'), $directiveName, json_encode($directiveArgs), implode($translationAPI->__('\', \'', 'pop-component-model'), $fieldDirectiveFields[$fieldDirective])),
                ];
                if ($stopDirectivePipelineExecutionIfDirectiveFailed) {
                    $schemaErrors[] = [
                        Tokens::PATH => [$fieldDirective],
            ...
            foreach ($fieldDirectiveFields[$enqueuedFieldDirective] as $field) {
                ...
                if (is_null($directiveResolverInstance)) {
                    $schemaErrors[] = [
                        Tokens::PATH => [$fieldDirective],
                        Tokens::MESSAGE => sprintf($translationAPI->__('No DirectiveResolver processes directive with name \'%s\' and arguments \'%s\' in field \'%s\'', 'pop-component-model'), $directiveName, json_encode($directiveArgs), $field),
                    ];
                    if ($stopDirectivePipelineExecutionIfDirectiveFailed) {
                        $schemaErrors[] = [
                            Tokens::PATH => [$fieldDirective],
        ...
        foreach ($directiveResolverInstanceFields as $instanceID => $instanceData) {
            ...
            if ($isRepeatedFieldDirective) {
                ...
                if (
                    !empty(array_filter($schemaErrors, function ($schemaError) use ($fieldDirective) {
                ...
                list(
                    ...
                    $directiveName,
                    $directiveArgs,
                ) = $directiveResolverInstance->dissectAndValidateDirectiveForSchema($this, $fieldDirectiveFields, $variables, $fieldSchemaErrors, $schemaWarnings, $schemaDeprecations, $schemaNotices, $schemaTraces);
            ...
            if (!$directiveResolverInstance->isRepeatable()) {
                ...
                if ($alreadyProcessingFields) {
                    ...
                    $schemaErrors[] = [
                        Tokens::PATH => [$fieldDirective],
                        Tokens::MESSAGE => sprintf($translationAPI->__('Directive \'%s\' can be executed only once for field(s) \'%s\'', 'component-model'), $fieldDirective, implode('\', \'', $alreadyProcessingFields)),
                    ];
                    if ($stopDirectivePipelineExecutionIfDirectiveFailed) {
                        $schemaErrors[] = [
                            Tokens::PATH => [$fieldDirective],
    ...
    protected function getUnresolvedResultItemIDError($resultItemID)
    {
        ...
        return new Error(
            ...
            sprintf($translationAPI->__('The DataLoader can\'t load data for object of type \'%s\' with ID \'%s\'', 'pop-component-model'), $this->getTypeOutputName(), $resultItemID)
        );
    }
    ...
    protected function addMandatoryDirectivesForDirectives(array $directives): array
    {
        ...
        foreach ($directives as $directive) {
            ...
            if (
                $mandatoryDirectivesForDirective = array_merge($precedingMandatoryDirectivesForDirectives[FieldSymbols::ANY_FIELD] ?? [], $precedingMandatoryDirectivesForDirectives[$directiveName] ?? [])
            ) {
            ...
            if (
                $mandatoryDirectivesForDirective = array_merge($succeedingMandatoryDirectivesForDirectives[FieldSymbols::ANY_FIELD] ?? [], $succeedingMandatoryDirectivesForDirectives[$directiveName] ?? [])
            ) {
                $allDirectives = array_merge($allDirectives, $this->addMandatoryDirectivesForDirectives($mandatoryDirectivesForDirective));
            }
        }
    ...
    protected function filterDirectiveNameClasses(array $directiveNameClasses): array
    {
        ...
        return array_filter($directiveNameClasses, function ($directiveName) use ($hooksAPI, $directiveNameClasses, $instanceManager) {
            ...
            foreach ($directiveResolverClasses as $directiveResolverClass) {
                ...
                if (
                    $hooksAPI->applyFilters(HookHelpers::getHookNameToFilterDirective(), true, $this, $directiveResolver, $directiveName)
                ) {
                    return $hooksAPI->applyFilters(HookHelpers::getHookNameToFilterDirective($directiveName), true, $this, $directiveResolver, $directiveName);
                }
                return false;
            }
            return true;
        ...
        foreach ($ids_data_fields as $id => $data_fields) {
            ...
            foreach ($fields as $field) {
                if (!isset($this->fieldDirectivesFromFieldCache[$field])) {
                    ...
                    $fieldName = $fieldQueryInterpreter->getFieldName($field);
                    if (
                        $mandatoryDirectivesForField = array_merge($mandatoryDirectivesForFields[FieldSymbols::ANY_FIELD] ?? [], $mandatoryDirectivesForFields[$fieldName] ?? [])
                    ) {
                    ...
                    $fieldDirectives = implode(QuerySyntax::SYMBOL_FIELDDIRECTIVE_SEPARATOR, array_map([$fieldQueryInterpreter, 'convertDirectiveToFieldDirective'], $directives));
                    // Assign in the cache
    ...
    public function resolveValue(
        ...
        if ($fieldResolvers = $this->getFieldResolversForField($field)) {
            ...
            $validateSchemaOnResultItem =
                ...
                FieldQueryUtils::isAnyFieldArgumentValueDynamic(array_values($fieldQueryInterpreter->extractFieldArguments($this, $field)));
    ...
    protected function isFieldNameResolvedByFieldResolver(FieldResolverInterface $fieldResolver, string $fieldName, array $fieldInterfaceResolverClasses): bool
    {
        ...
        $hooksAPI = HooksAPIFacade::getInstance();
        if (
            $hooksAPI->applyFilters(HookHelpers::getHookNameToFilterField(), true, $this, $fieldResolver, $fieldInterfaceResolverClassesForField, $fieldName)
        ) {
            return $hooksAPI->applyFilters(HookHelpers::getHookNameToFilterField($fieldName), true, $this, $fieldResolver, $fieldInterfaceResolverClassesForField, $fieldName);
        }
        return false;
    }
