...
class QueryableFieldInterfaceResolver extends AbstractSchemaFieldInterfaceResolver
{
    ...
    {
        $types = ['url' => SchemaDefinition::TYPE_URL, 'slug' => SchemaDefinition::TYPE_STRING];
        return $types[$fieldName] ?? parent::getSchemaFieldType($fieldName);
    }
    public function getSchemaFieldDescription(string $fieldName) : ?string
    {
        $descriptions = ['url' => $this->translationAPI->__('URL to query the object', 'queriedobject'), 'slug' => $this->translationAPI->__('URL\'s slug', 'queriedobject')];
