...
class GraphQLQueryConvertor implements \GraphQLByPoP\GraphQLQuery\Schema\GraphQLQueryConvertorInterface
{
    ...
    {
        list($operationType, $operationFieldQueryPaths) = $this->convertFromGraphQLToFieldQueryPaths($graphQLQuery, $variables ?? [], $enableMultipleQueryExecution, $operationName);
        $fieldQueries = [];
     ...
     */
    protected function convertFromGraphQLToFieldQueryPaths(string $graphQLQuery, array $variables, bool $enableMultipleQueryExecution, ?string $operationName = null) : array
    {
        try {
        ...
        return [$operationType, $fieldQueryPaths];
    }
    /**
    ...
    protected function convertField(FieldInterface $field) : string
    {
        ...
        foreach (\array_reverse($fieldDirectives) as $directive) {
            ...
            if ($enableComposableDirectives) {
                 ...
                 */
                if (isset($directiveArgs[\GraphQLByPoP\GraphQLQuery\Schema\SchemaElements::DIRECTIVE_PARAM_NESTED_UNDER])) {
                    $nestedUnder = $directiveArgs[\GraphQLByPoP\GraphQLQuery\Schema\SchemaElements::DIRECTIVE_PARAM_NESTED_UNDER];
                    unset($directiveArgs[\GraphQLByPoP\GraphQLQuery\Schema\SchemaElements::DIRECTIVE_PARAM_NESTED_UNDER]);
                }
                /**
            ...
            if ($enableComposableDirectives && $nestedUnder !== null) {
                if (!\is_int($nestedUnder) || !($nestedUnder < 0)) {
                    $this->feedbackMessageStore->addQueryError(\sprintf($this->translationAPI->__('Param \'%s\' must be a negative integer, hence value \'%s\' in directive \'%s\' has been ignored', 'graphql-query'), \GraphQLByPoP\GraphQLQuery\Schema\SchemaElements::DIRECTIVE_PARAM_NESTED_UNDER, $nestedUnder, $directiveName));
                } elseif (-1 * $nestedUnder > $counter) {
                    $this->feedbackMessageStore->addQueryError(\sprintf($this->translationAPI->__('There is no directive at position \'%s\' (set under param \'%s\') relative to directive \'%s\'', 'graphql-query'), $nestedUnder, \GraphQLByPoP\GraphQLQuery\Schema\SchemaElements::DIRECTIVE_PARAM_NESTED_UNDER, $directiveName));
                } else {
                    // From the current position, move "$nestedUnder" positions to the left
                    // (it's a negative int)
                    $nestedUnderPos = $counter + $nestedUnder;
                    $composableDirectivesByPosition[$nestedUnderPos] = $composableDirectivesByPosition[$nestedUnderPos] ?? [];
        ...
        foreach ($fields as $field) {
            if ($field instanceof Field) {
                ...
                foreach ($fragmentConvertedFieldPaths as $fragmentFieldPath) {
                    $queryFieldPaths[] = \array_merge($queryFieldPath, $fragmentFieldPath);
                }
            }
        }
    }
    ...
    protected function convertRequestToFieldQueryPaths(Request $request) : array
    {
        ...
        if ($mutations) {
            ...
            $operationType = \GraphQLByPoP\GraphQLQuery\Schema\OperationTypes::MUTATION;
        } else {
            $queriesOrMutations = $queries;
            $operationType = \GraphQLByPoP\GraphQLQuery\Schema\OperationTypes::QUERY;
        }
        ...
        if ($queries && $mutations) {
            $this->feedbackMessageStore->addQueryWarning($this->translationAPI->__('Cannot execute both queries AND mutations, hence the queries have been ignored, resolving mutations only', 'graphql-query'));
        }
        foreach ($queriesOrMutations as $query) {
            $operationLocation = $query->getLocation();
            $operationID = \sprintf('%s-%s', $operationLocation->getLine(), $operationLocation->getColumn());
            $fieldQueryPaths[$operationID] = \array_merge($fieldQueryPaths[$operationID] ?? [], $this->getFieldPathsFromQuery($request, $query));
        }
        return [$operationType, $fieldQueryPaths];
    }
    /**
    ...
    protected function parseAndCreateRequest(string $payload, array $variables, bool $enableMultipleQueryExecution, ?string $operationName = null) : Request
    {
        ...
        if (\is_null($operationName)) {
             ...
             * ```
             */
            if ($enableMultipleQueryExecution && \strtoupper($operationName) == \GraphQLByPoP\GraphQLQuery\Schema\ClientSymbols::GRAPHIQL_QUERY_BATCHING_OPERATION_NAME) {
                ...
                foreach ($parsedData['queryOperations'] as $queryOperation) {
                    if ($queryOperation['name'] == $operationName) {
                        \array_splice($parsedData['queries'], $queryOperation['position'], $queryOperation['numberItems']);
                        break;
                    }
                }
                foreach ($parsedData['mutationOperations'] as $mutationOperation) {
                    if ($mutationOperation['name'] == $operationName) {
                        \array_splice($parsedData['mutations'], $mutationOperation['position'], $mutationOperation['numberItems']);
                        break;
                    }
                }
            } else {
                ...
                if ($parsedData['queryOperations']) {
                    for ($i = \count($parsedData['queryOperations']) - 1; $i >= 0; $i--) {
                        ...
                        if ($queryOperation['name'] == $operationName) {
                            $parsedData['queries'] = \array_slice($parsedData['queries'], $queryOperation['position'], $queryOperation['numberItems']);
                            break;
                        } else {
                            \array_splice($parsedData['queries'], $queryOperation['position'], $queryOperation['numberItems']);
                        }
                ...
                if ($parsedData['mutationOperations']) {
                    for ($i = \count($parsedData['mutationOperations']) - 1; $i >= 0; $i--) {
                        ...
                        if ($mutationOperation['name'] == $operationName) {
                            $parsedData['mutations'] = \array_slice($parsedData['mutations'], $mutationOperation['position'], $mutationOperation['numberItems']);
                            break;
                        } else {
                            \array_splice($parsedData['mutations'], $mutationOperation['position'], $mutationOperation['numberItems']);
                        }
