...
EOF;
        ...
        if ($schemaConfigurationID == self::ATTRIBUTE_VALUE_SCHEMA_CONFIGURATION_DEFAULT) {
            ...
            if (!is_null($schemaConfigurationObject)) {
                ...
                $schemaConfigurationContent = ($permalink ?
                    \sprintf('<code><a href="%s">%s</a></code>', $permalink, BlockRenderingHelpers::getCustomPostTitle($schemaConfigurationObject)) :
                    \sprintf('<code>%s</code>', BlockRenderingHelpers::getCustomPostTitle($schemaConfigurationObject))
        ...
        return sprintf($blockContentPlaceholder, $className, $className . '-front', \__('Schema Configuration', 'graphql-api'), $schemaConfigurationContent);
    }
}
