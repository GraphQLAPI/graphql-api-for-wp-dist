...
abstract class AbstractContentParser implements ContentParserInterface
{
    ...
    public function getContent(
        ...
        if (file_exists($localizeFile)) {
            ...
            if (!file_exists($file)) {
                throw new InvalidArgumentException(sprintf(\__('File \'%s\' does not exist', 'graphql-api'), $file));
            }
        }
        $fileContent = file_get_contents($file);
        if ($fileContent === false) {
            throw new InvalidArgumentException(sprintf(\__('File \'%s\' is corrupted', 'graphql-api'), $file));
        }
    ...
    protected function tabContent(string $htmlContent): string
    {
        ...
        if ($firstTagPos !== false) {
            ...
            $contentStarter = substr($htmlContent, 0, $firstTagPos);
            // Add the markup for the tabs around every <h2>
            $regex = sprintf('/<%1$s>(.*?)<\/%1$s>/', $tag);
            $headers = [];
            $panelContent = preg_replace_callback($regex, function ($matches) use (&$headers) {
                ...
                $headers[] = $matches[1];
                return $tabbedPanel . sprintf('<div id="doc-panel-%s" class="tab-content" style="display: %s;">', count($headers), $isFirstTab ? 'block' : 'none');// . $matches[0];
            }, substr($htmlContent, $firstTagPos)) . '</div>';
            ...
            for ($i = 0; $i < $headersCount; $i++) {
                $isFirstTab = $i == 0;
                $panelTabs .= sprintf('<a href="#doc-panel-%s" class="nav-tab %s">%s</a>', $i + 1, $isFirstTab ? 'nav-tab-active' : '', $headers[$i]);
            }
            $panelTabs .= '</h2>';
    ...
    {
        return (string)preg_replace_callback('/<a.*href="(.*?)\.md".*?>/', function (array $matches): string {
            ...
            /** @var string */
            $link = str_replace("href=\"{$matches[1]}.md\"", "href=\"{$elementURL}\"", $matches[0]);
            // Must also add some classnames
            $classnames = 'thickbox open-plugin-details-modal';
            // 1. If there are classes already
            /** @var string */
            $replacedLink = preg_replace_callback('/ class="(.*?)"/', function (array $matches) use ($classnames): string {
                return str_replace(" class=\"{$matches[1]}\"", " class=\"{$matches[1]} {$classnames}\"", $matches[0]);
            }, $link);
            // 2. If there were no classes
            if ($replacedLink == $link) {
                $replacedLink = str_replace("<a ", "<a class=\"{$classnames}\" ", $link);
            }
            return $replacedLink;
        }, $htmlContent);
    }
    ...
    protected function embedVideos(string $htmlContent): string
    {
        ...
        return (string)preg_replace_callback('/<p>(.*?)<a href="https:\/\/(vimeo.com|youtube.com|youtu.be)\/(.*?)">(.*?)<\/a>\.?<\/p>/', function ($matches) {
            global $wp_embed;
            // Keep the link, and append the embed immediately after
            return
                $matches[0]
                . '<div class="video-responsive-container">' .
                    $wp_embed->autoembed(sprintf('https://%s/%s', $matches[2], $matches[3]))
                . '</div>';
        }, $htmlContent);
    }
    ...
    protected function addClasses(string $htmlContent): string
    {
         ...
         */
        return str_replace('<table>', '<table class="wp-list-table widefat striped">', $htmlContent);
    }
    ...
    protected function appendPathURLToElement(string $tag, string $attr, string $pathURL, string $htmlContent): string
    {
        ...
        return (string)preg_replace_callback($regex, function ($matches) use ($pathURL, $attr) {
            // If the element has an absolute route, then no need
            if ($this->isAbsoluteURL($matches[1]) || $this->isMailto($matches[1])) {
                return $matches[0];
            }
            $elementURL = \trailingslashit($pathURL) . $matches[1];
            return str_replace("{$attr}=\"{$matches[1]}\"", "{$attr}=\"{$elementURL}\"", $matches[0]);
        }, $htmlContent);
    }
}
