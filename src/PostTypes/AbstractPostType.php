...
abstract class AbstractPostType
{
    ...
    public function initialize(): void
    {
        ...
        \add_action('init', [$this, 'maybeLockGutenbergTemplate']);
        \add_filter('allowed_block_types', [$this, 'allowGutenbergBlocksForCustomPostType'], 10, 2);
        ...
        if ($this->isExcerptAsDescriptionEnabled() && $this->usePostExcerptAsDescription()) {
            ...
            \add_filter('the_content', [$this, 'maybeAddExcerptAsDescription'], PHP_INT_MAX);
            // Add the custom columns to the post type
            add_filter("manage_{$postType}_posts_columns", [$this, 'setTableColumns']);
            add_action("manage_{$postType}_posts_custom_column", [$this, 'resolveCustomColumn'], 10, 2);
        }
         ...
         */
        \add_filter('post_row_actions', [$this, 'maybeAddPostTypeTableActions'], 10, 2);
        \add_filter('page_row_actions', [$this, 'maybeAddPostTypeTableActions'], 10, 2);
        ...
        if ($this->regenerateTimestampOnSave()) {
            ...
            // but can't avoid it
            \add_action("save_post_{$postType}", function ($postID, $post): void {
    ...
    public function setTableColumns(array $columns): array
    {
        ...
        return array_merge(array_slice($columns, 0, $titlePos + 1, true), [
            'description' => \__('Description', 'graphql-api'),
        ], array_slice($columns, $titlePos + 1, null, true));
    }
    ...
    public function maybeAddPostTypeTableActions(array $actions, $post): array
    {
        if ($post->post_type == $this->getPostType()) {
            $actions = \array_merge($actions, $this->getPostTypeTableActions($post));
        }
        return $actions;
    }
        ...
        if (
            ...
            if ($customPost !== null) {
                if ($excerpt = CPTUtils::getCustomPostDescription($customPost)) {
                    $content = \sprintf(\__('<p class="%s"><strong>Description: </strong>%s</p>'), $this->getAlignClass(), $excerpt) . $content;
                }
            }
        }
        return $content;
    ...
    protected function getPostTypeArgs(): array
    {
        ...
        $postTypeArgs = array_merge($securityPostTypeArgs, array(
    ...
    protected function getGutenbergBlocksForCustomPostType(): array
    {
        ...
        if (!empty($template) && $this->enableOnlyGutenbergTemplateBlocks()) {
            ...
            return array_values(array_unique(array_map(function (array $blockConfiguration) {
