...
class Interval
{
    ...
    /** @var Constraint */
    private $end;
    public function __construct(\PrefixedByPoP\Composer\Semver\Constraint\Constraint $start, \PrefixedByPoP\Composer\Semver\Constraint\Constraint $end)
    {
        $this->start = $start;
        $this->end = $end;
    }
    ...
    public static function fromZero()
    {
        static $zero;
        if (null === $zero) {
            $zero = new \PrefixedByPoP\Composer\Semver\Constraint\Constraint('>=', '0.0.0.0-dev');
        }
        return $zero;
    }
    /**
    ...
    public static function untilPositiveInfinity()
    {
        ...
        if (null === $positiveInfinity) {
            $positiveInfinity = new \PrefixedByPoP\Composer\Semver\Constraint\Constraint('<', \PHP_INT_MAX . '.0.0.0');
        }
