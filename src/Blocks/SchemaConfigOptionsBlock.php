...
class SchemaConfigOptionsBlock extends AbstractOptionsBlock
{
    ...
    public function renderBlock(array $attributes, string $content): string
    {
        ...
        if ($moduleRegistry->isModuleEnabled(SchemaConfigurationFunctionalityModuleResolver::PUBLIC_PRIVATE_SCHEMA)) {
            ...
            $blockContent .= sprintf($blockContentPlaceholder, \__('Public/Private Schema:', 'graphql-api'), $schemaModeLabels[$attributes[self::ATTRIBUTE_NAME_DEFAULT_SCHEMA_MODE] ?? ''] ?? ComponentConfiguration::getSettingsValueLabel());
        }
        ...
        if ($moduleRegistry->isModuleEnabled(SchemaConfigurationFunctionalityModuleResolver::SCHEMA_NAMESPACING)) {
            ...
            $blockContent .= sprintf($blockContentPlaceholder, \__('Use namespacing?', 'graphql-api'), $useNamespacingLabels[$attributes[self::ATTRIBUTE_NAME_USE_NAMESPACING] ?? ''] ?? ComponentConfiguration::getSettingsValueLabel());
        }
        ...
        if ($moduleRegistry->isModuleEnabled(OperationalFunctionalityModuleResolver::NESTED_MUTATIONS)) {
            ...
            $blockContent .= sprintf($blockContentPlaceholder, \__('Mutation Scheme', 'graphql-api'), $mutationSchemeLabels[$attributes[self::ATTRIBUTE_NAME_MUTATION_SCHEME] ?? ''] ?? ComponentConfiguration::getSettingsValueLabel());
        }

        // If all modules are disabled and there are not options...
        if ($blockContent == '') {
            $blockContent = sprintf('<p>%s</p>', \__('(Nothing here: All Schema Configuration options are disabled)', 'graphql-api'));
        }
...
EOT;
        return sprintf($blockContentPlaceholder, $className . ' ' . $this->getAlignClass(), $className . '__title', \__('Options', 'graphql-api'), $blockContent);
    }
