...
class MenuTypeAPI implements MenuTypeAPIInterface
{
    ...
    public function getMenuItems($menuObjectOrID): array
    {
        ...
        return array_map(
            function ($menuItem): MenuItem {
                return new MenuItem($menuItem->ID, (int) $menuItem->object_id, $menuItem->menu_item_parent === "0" ? null : (int) $menuItem->menu_item_parent, App::applyFilters('the_title', $menuItem->title, $menuItem->object_id), $menuItem->attr_title, $menuItem->url, $menuItem->description, App::applyFilters('menuitem:classes', array_filter($menuItem->classes), $menuItem), $menuItem->target, $menuItem->xfn);
            },
            $menuItems
        );
    }
     ...
     * @param string $menuName
     */
    protected function getMenuObject($menuName): ?WP_Term
    {
     ...
     * @param array<string,mixed> $options
     */
    protected function convertMenusQuery($query, $options = []): array
    {
