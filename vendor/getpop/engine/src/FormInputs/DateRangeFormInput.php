...
class DateRangeFormInput extends \PoP\Engine\FormInputs\MultipleInputFormInput
{
    public function __construct($params = array())
    {
        ...
        if (!isset($params['selected']) && $this->selected) {
            ...
            if ($selected['from'] && $selected['to']) {
                ...
                $selected['readable'] = \sprintf(TranslationAPIFacade::getInstance()->__('%1$s - %2$s', 'engine'), \date('m/d/Y', \strtotime($selected['from'])), \date('m/d/Y', \strtotime($selected['to'])));
            }
        }
    }
}
