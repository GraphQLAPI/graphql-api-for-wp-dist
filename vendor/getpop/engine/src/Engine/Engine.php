...
class Engine extends \PoP\ComponentModel\Engine\Engine implements \PoP\Engine\Engine\EngineInterface
{
    public function generateData()
    {
        ...
        $looseContractManager = \PoP\LooseContracts\Facades\LooseContractManagerFacade::getInstance();
        $translationAPI = \PoP\Translation\Facades\TranslationAPIFacade::getInstance();
        if ($notImplementedHooks = $looseContractManager->getNotImplementedRequiredHooks()) {
            throw new \Exception(\sprintf($translationAPI->__('The following hooks have not been implemented by the CMS: "%s". Hence, we can\'t continue.'), \implode($translationAPI->__('", "'), $notImplementedHooks)));
        }
        if ($notImplementedNames = $looseContractManager->getNotImplementedRequiredNames()) {
            throw new \Exception(\sprintf($translationAPI->__('The following names have not been implemented by the CMS: "%s". Hence, we can\'t continue.'), \implode($translationAPI->__('", "'), $notImplementedNames)));
        }
        parent::generateData();
    }
    public function maybeRedirectAndExit() : void
    {
        if ($redirect = \PoP\ComponentModel\Settings\SettingsManagerFactory::getInstance()->getRedirectUrl()) {
    ...
    public function outputResponse() : void
    {
        ...
        $headers = [];
        if (\PoP\CacheControl\Component::isEnabled()) {
            $cacheControlEngine = \PoP\CacheControl\Facades\CacheControlEngineFacade::getInstance();
