...
abstract class AbstractBlock
{
    ...
    {
        return sprintf('<p>%s</p>', \__('You are not authorized to see this content', 'graphql-api'));
    }
    /**
     * Register index.css
     *
    ...
    {
        return sprintf('%s/%s', $this->getBlockNamespace(), $this->getBlockName());
    }
    /**
    ...
    {
        return sprintf('wp-block-%s', $this->getBlockRegistrationName());
    }
    ...
    public function initBlock(): void
    {
        ...
        if (!file_exists($script_asset_path)) {
            throw new Error(
                sprintf(\__('You need to run `npm start` or `npm run build` for the "%s" block first.', 'graphql-api'), $blockFullName)
            );
        }
        ...
        \wp_register_script($scriptRegistrationName, $url . $index_js, array_merge($script_asset['dependencies'], $this->getBlockDependencies()), $script_asset['version']);
        ...
        \add_action('wp_print_scripts', function () use ($scriptRegistrationName) {
            if ($localizedData = $this->getLocalizedData()) {
                \wp_localize_script($scriptRegistrationName, $this->getBlockLocalizationName(), $localizedData);
            }
        });
