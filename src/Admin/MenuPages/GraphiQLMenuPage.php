...
class GraphiQLMenuPage extends AbstractMenuPage
{
    ...
    protected function enqueueAssets(): void
    {
        ...
        // CSS
        \wp_enqueue_style('graphql-api-graphiql-client', \GRAPHQL_API_URL . 'assets/css/graphiql-client.css', array(), \GRAPHQL_API_VERSION);
        ...
        if (!$useGraphiQLExplorer) {
            \wp_enqueue_style('graphql-api-graphiql', \GRAPHQL_API_URL . 'assets/css/vendors/graphiql.min.css', array(), \GRAPHQL_API_VERSION);

            // JS: execute them all in the footer
            $this->enqueueReactAssets(true);

            \wp_enqueue_script('graphql-api-graphiql', \GRAPHQL_API_URL . 'assets/js/vendors/graphiql.min.js', array('graphql-api-react-dom'), \GRAPHQL_API_VERSION, true);
            \wp_enqueue_script('graphql-api-graphiql-client', \GRAPHQL_API_URL . 'assets/js/graphiql-client.js', array('graphql-api-graphiql'), \GRAPHQL_API_VERSION, true);

            // Load data into the script
            \wp_localize_script('graphql-api-graphiql-client', 'graphQLByPoPGraphiQLSettings', array_merge([
            ...
            foreach ($cssFileURLs as $index => $cssFileURL) {
                \wp_enqueue_style('graphql-api-graphiql-with-explorer-' . $index, $cssFileURL, array(), \GRAPHQL_API_VERSION);
            }
            preg_match_all('/<script[^>]+src="([^">]+)"/s', $htmlContent, $matches);
            $jsFileURLs = $matches[1];
            foreach ($jsFileURLs as $index => $jsFileURL) {
                \wp_enqueue_script('graphql-api-graphiql-with-explorer-' . $index, $jsFileURL, array(), \GRAPHQL_API_VERSION, true);
            }

            // Override styles for the admin, so load last
            \wp_enqueue_style('graphql-api-graphiql-with-explorer-client', \GRAPHQL_API_URL . 'assets/css/graphiql-with-explorer-client.css', array(), \GRAPHQL_API_VERSION);

            // Load data into the script. Because no script is enqueued since it is
            // in the body, then localize it to React
            \wp_localize_script('graphql-api-graphiql-with-explorer-0', 'graphiQLWithExplorerClientForWP', $scriptSettings);
        }
    }
