...
class PersistedQueryOptionsBlock extends AbstractQueryExecutionOptionsBlock
{
    ...
    protected function getBlockContent(array $attributes, string $content): string
    {
        ...
        $blockContent .= sprintf($blockContentPlaceholder, \__('Accept variables as URL params:', 'graphql-api'), $this->getBooleanLabel($attributes[self::ATTRIBUTE_NAME_ACCEPT_VARIABLES_AS_URL_PARAMS] ?? true));
        $blockContent .= sprintf($blockContentPlaceholder, \__('Inherit query from ancestor(s):', 'graphql-api'), $this->getBooleanLabel($attributes[self::ATTRIBUTE_NAME_INHERIT_QUERY] ?? false));
