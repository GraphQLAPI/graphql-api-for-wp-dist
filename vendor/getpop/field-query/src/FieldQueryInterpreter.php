...
class FieldQueryInterpreter implements FieldQueryInterpreterInterface
{
    ...
    protected function doGetVariablesFromRequest(): array
    {
        ...
        return array_merge($_REQUEST, isset($_REQUEST['variables']) && is_array($_REQUEST['variables']) ?
            $_REQUEST['variables']
            : []);
    }
    ...
    protected function doGetFieldArgs(string $field): ?string
    {
        ...
        if (
            ...
            $this->feedbackMessageStore->addQueryError(sprintf($this->translationAPI->__('Arguments \'%s\' must start with symbol \'%s\' and end with symbol \'%s\'', 'component-model'), $field, QuerySyntax::SYMBOL_FIELDARGS_OPENING, QuerySyntax::SYMBOL_FIELDARGS_CLOSING));
            return null;
        }

        // We have field args. Extract them, including the brackets
        return substr($field, (int)$fieldArgsOpeningSymbolPos, $fieldArgsClosingSymbolPos + strlen(QuerySyntax::SYMBOL_FIELDARGS_CLOSING) - $fieldArgsOpeningSymbolPos);
    }
    ...
    public function removeSkipOuputIfNullFromField(string $field): string
    {
        ...
        if ($pos !== false) {
            ...
            $field = str_replace(QuerySyntax::SYMBOL_SKIPOUTPUTIFNULL, '', $field);
        }
        return $field;
    }
    ...
    protected function replaceFieldArgs(string $field, array $fieldArgs = []): string
    {
        ...
        if ($fieldArgsOpeningSymbolPos !== false && $fieldArgsClosingSymbolPos !== false) {
            ...
            return substr($field, 0, $fieldArgsOpeningSymbolPos) .
            $this->getFieldArgsAsString($fieldArgs) .
            substr($field, $fieldArgsClosingSymbolPos + strlen(QuerySyntax::SYMBOL_FIELDDIRECTIVE_CLOSING));
        }
    ...
    public function isFieldArgumentValueAnExpression($fieldArgValue): bool
    {
        ...
        return
            is_string($fieldArgValue)
            && substr($fieldArgValue, 0, strlen(QuerySyntax::SYMBOL_EXPRESSION_OPENING)) == QuerySyntax::SYMBOL_EXPRESSION_OPENING
            && substr($fieldArgValue, -1 * strlen(QuerySyntax::SYMBOL_EXPRESSION_CLOSING)) == QuerySyntax::SYMBOL_EXPRESSION_CLOSING;
    }
    ...
    public function isFieldArgumentValueAVariable($fieldArgValue): bool
    {
        ...
        return
            is_string($fieldArgValue)
            && substr($fieldArgValue, 0, strlen(QuerySyntax::SYMBOL_VARIABLE_PREFIX)) == QuerySyntax::SYMBOL_VARIABLE_PREFIX;
    }
    ...
    public function isFieldArgumentValueAnArrayRepresentedAsString($fieldArgValue): bool
    {
        ...
        return
            is_string($fieldArgValue)
            && substr($fieldArgValue, 0, strlen(QuerySyntax::SYMBOL_FIELDARGS_ARGVALUEARRAY_OPENING)) == QuerySyntax::SYMBOL_FIELDARGS_ARGVALUEARRAY_OPENING
            && substr($fieldArgValue, -1 * strlen(QuerySyntax::SYMBOL_FIELDARGS_ARGVALUEARRAY_CLOSING)) == QuerySyntax::SYMBOL_FIELDARGS_ARGVALUEARRAY_CLOSING;
    }
    ...
    protected function doGetFieldAlias(string $field): ?string
    {
        if ($fieldAliasPositionSpan = $this->getFieldAliasPositionSpanInField($field)) {
            ...
            return substr($field, $aliasSymbolPos + strlen(QuerySyntax::SYMBOL_FIELDALIAS_PREFIX), $aliasSymbolLength - strlen(QuerySyntax::SYMBOL_FIELDALIAS_PREFIX));
        }
        return null;
    }
    ...
    protected function doGetFieldDirectives(string $field): ?string
    {
        ...
        if (
            ...
            $this->feedbackMessageStore->addQueryError(sprintf($this->translationAPI->__('Directive \'%s\' must start with symbol \'%s\' and end with symbol \'%s\'', 'component-model'), $field, QuerySyntax::SYMBOL_FIELDDIRECTIVE_OPENING, QuerySyntax::SYMBOL_FIELDDIRECTIVE_CLOSING));
            return null;
        }
    ...
    protected function doExtractFieldDirectives(string $fieldDirectives): array
    {
        ...
        return array_map([$this, 'listFieldDirective'], $this->queryParser->splitElements($fieldDirectives, QuerySyntax::SYMBOL_FIELDDIRECTIVE_SEPARATOR, [
            ...
            QuerySyntax::SYMBOL_FIELDDIRECTIVE_CLOSING
        ], QuerySyntax::SYMBOL_FIELDARGS_ARGVALUESTRING_OPENING, QuerySyntax::SYMBOL_FIELDARGS_ARGVALUESTRING_CLOSING));
    }
    ...
    public function getArrayAsStringForQuery(array $fieldArgValue): string
    {
        ...
        return
            ...
            implode(QuerySyntax::SYMBOL_FIELDARGS_ARGVALUEARRAY_SEPARATOR, $elems) .
    ...
    public function getFieldDirectivesAsString(array $fieldDirectives): string
    {
        ...
        return
            QuerySyntax::SYMBOL_FIELDDIRECTIVE_OPENING .
            implode(QuerySyntax::SYMBOL_FIELDDIRECTIVE_SEPARATOR, array_map(function ($fieldDirective) {
