...
class EarlyExpirationDispatcher
{
    ...
    public function __construct(\PrefixedByPoP\Symfony\Component\Messenger\MessageBusInterface $bus, \PrefixedByPoP\Symfony\Component\DependencyInjection\ReverseContainer $reverseContainer, callable $callbackWrapper = null)
    {
        $this->bus = $bus;
        $this->reverseContainer = $reverseContainer;
        $this->callbackWrapper = $callbackWrapper;
    }
    public function __invoke(callable $callback, \PrefixedByPoP\Symfony\Component\Cache\CacheItem $item, bool &$save, \PrefixedByPoP\Symfony\Component\Cache\Adapter\AdapterInterface $pool, \Closure $setMetadata, \PrefixedByPoP\Psr\Log\LoggerInterface $logger = null)
    {
        if (!$item->isHit() || null === ($message = \PrefixedByPoP\Symfony\Component\Cache\Messenger\EarlyExpirationMessage::create($this->reverseContainer, $callback, $item, $pool))) {
        ...
        if ($logger) {
            if ($envelope->last(\PrefixedByPoP\Symfony\Component\Messenger\Stamp\HandledStamp::class)) {
