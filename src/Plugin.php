...
class Plugin
{
    ...
    public function setup(): void
    {
         ...
         */
        \register_activation_hook(\GRAPHQL_API_PLUGIN_FILE, function (): void {
        ...
        \add_action('admin_init', function (): void {
            ...
            if (
                !$userSettingsManager->getSetting(PluginManagementFunctionalityModuleResolver::GENERAL, PluginManagementFunctionalityModuleResolver::OPTION_ADD_RELEASE_NOTES_ADMIN_NOTICE)
            ) {
                return;
            }
    ...
    protected function showReleaseNotesInAdminNotice(): void
    {
        ...
        \add_action('admin_notices', function () {
            ...
            $releaseNotesURL = \admin_url(sprintf('admin.php?page=%s&%s=%s&%s=%s&TB_iframe=true', $aboutMenuPage->getScreenID(), RequestParams::TAB, RequestParams::TAB_DOCS, RequestParams::DOC, sprintf('release-notes/%s', $minorReleaseVersion)));
            /**
             * @var SettingsMenuPage
             */
            $settingsMenuPage = $instanceManager->getInstance(SettingsMenuPage::class);
            $moduleRegistry = ModuleRegistryFacade::getInstance();
            $generalSettingsURL = \admin_url(sprintf('admin.php?page=%s&tab=%s', $settingsMenuPage->getScreenID(), $moduleRegistry->getModuleResolver(PluginManagementFunctionalityModuleResolver::GENERAL)->getID(PluginManagementFunctionalityModuleResolver::GENERAL)));
            _e(sprintf('<div class="notice notice-success is-dismissible">' .
            '<p>%s</p>' .
            '</div>', sprintf(__('Plugin <strong>GraphQL API for WordPress</strong> has been updated to version <code>%s</code>. <strong><a href="%s" class="%s">Check out what\'s new</a></strong> | <a href="%s">Disable this admin notice in the Settings</a>', 'graphql-api'), \GRAPHQL_API_VERSION, $releaseNotesURL, 'thickbox open-plugin-details-modal', $generalSettingsURL)));
        });
    }
    ...
    public function deactivate(): void
    {
        ...
        $instanceManager = InstanceManagerFacade::getInstance();
        $postTypeObjects = array_map(function ($serviceClass) use ($instanceManager): AbstractPostType {
            /**
             * @var AbstractPostType
             */
