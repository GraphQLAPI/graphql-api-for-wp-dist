...
class Engine implements EngineInterface
{
    ...
    public function getExtraRoutes(): array
    {
        ...
        $this->extra_routes = HooksAPIFacade::getInstance()->applyFilters('\PoP\ComponentModel\Engine:getExtraRoutes', $this->extra_routes);
    ...
    protected function processAndGenerateData()
    {
        ...
        HooksAPIFacade::getInstance()->doAction('\PoP\ComponentModel\Engine:helperCalculations', array(&$this->helperCalculations), $module, array(&$this->props));
    ...
    public function getRequestMeta()
    {
        ...
        return HooksAPIFacade::getInstance()->applyFilters('\PoP\ComponentModel\Engine:request-meta', $meta);
    }

    public function getSessionMeta()
    {
        return HooksAPIFacade::getInstance()->applyFilters('\PoP\ComponentModel\Engine:session-meta', array());
    }
    ...
    public function getSiteMeta()
    {
        ...
        return HooksAPIFacade::getInstance()->applyFilters('\PoP\ComponentModel\Engine:site-meta', $meta);
    }
    ...
    private function combineIdsDatafields(&$typeResolver_ids_data_fields, $typeResolver_class, $ids, $data_fields, $conditional_data_fields = [])
    {
        ...
        foreach ($ids as $id) {
            ...
            $typeResolver_ids_data_fields[$typeResolver_class][(string)$id]['direct'] = array_values(array_unique(array_merge($typeResolver_ids_data_fields[$typeResolver_class][(string)$id]['direct'], $data_fields ?? array())));
            ...
            foreach ($conditional_data_fields as $conditionDataField => $conditionalDataFields) {
                $typeResolver_ids_data_fields[$typeResolver_class][(string)$id]['conditional'][$conditionDataField] = array_merge($typeResolver_ids_data_fields[$typeResolver_class][(string)$id]['conditional'][$conditionDataField] ?? [], $conditionalDataFields);
            }
        }
    }
    ...
    private function doAddDatasetToDatabase(&$database, $database_key, $dataitems)
    {
        ...
        if (!isset($database[$database_key])) {
            ...
            foreach ($dataitems as $id => $dbobject_values) {
                ...
                $database[$dbKey][(string)$id] = array_merge($database[$dbKey][(string)$id], $dbobject_values);
            }
        }
    }
    ...
    private function addInterreferencedModuleFullpaths(&$paths, $module_path, array $module, array &$props)
    {
        ...
        if (!$modulefilter_manager->excludeModule($module, $props)) {
            ...
            if ($interreferenced_modulepath = $processor->getDataFeedbackInterreferencedModulepath($module, $props)) {
                ...
                $paths[$referenced_modulepath][] = array_merge($module_path, array(
                    $module
                ));
            }
        }

        $submodule_path = array_merge($module_path, array(
            $module,
        ));
    ...
    private function addDataloadingModuleFullpaths(&$paths, $module_path, array $module, array &$props)
    {
        ...
        if (!$modulefilter_manager->excludeModule($module, $props)) {
            ...
            if ($processor->moduleLoadsData($module)) {
                $paths[] = array_merge($module_path, array(
                    $module
                ));
            }
        }
    ...
    public function getModuleData($root_module, $root_model_props, $root_props)
    {
        ...
        HooksAPIFacade::getInstance()->doAction('\PoP\ComponentModel\Engine:getModuleData:start', $root_module, array(&$root_model_props), array(&$root_props), array(&$this->helperCalculations), $this);
        ...
        foreach ($module_fullpaths as $module_path) {
            ...
            if ($load_data) {
                ...
                if ($load_data) {
                    ...
                    $this->dbdata[$typeResolver_class][$module_path_key]['ids'] = array_merge($this->dbdata[$typeResolver_class][$module_path_key]['ids'], $typeDBObjectIDs);
            ...
            // Allow PoP UserState to add the lazy-loaded userstate data triggers
            HooksAPIFacade::getInstance()->doAction('\PoP\ComponentModel\Engine:getModuleData:dataloading-module', $module, array(&$module_props), array(&$data_properties), $dataaccess_checkpoint_validation, $mutation_checkpoint_validation, $executed, $dbObjectIDOrIDs, array(&$this->helperCalculations), $this);
        }
        ...
        if (in_array(GD_URLPARAM_DATAOUTPUTITEMS_MODULEDATA, $dataoutputitems)) {
            ...
            if ($dataoutputmode == GD_URLPARAM_DATAOUTPUTMODE_SPLITBYSOURCES) {
                ...
                // If everything is combined, then it belongs under "mutableonrequest"
                if (
                    $combined_moduledata = array_merge_recursive($immutable_moduledata ?? array(), $mutableonmodel_moduledata ?? array(), $mutableonrequest_moduledata ?? array())
                ) {
                    $ret['moduledata'] = $has_extra_routes ? array($current_uri => $combined_moduledata) : $combined_moduledata;
                }
                if (
                    $combined_datasetmoduledata = array_merge_recursive($immutable_datasetmoduledata ?? array(), $mutableonmodel_datasetmoduledata ?? array(), $mutableonrequest_datasetmoduledata ?? array())
                ) {
                    $ret['datasetmoduledata'] = $has_extra_routes ? array($current_uri => $combined_datasetmoduledata) : $combined_datasetmoduledata;
                }
                if ($add_meta) {
                    if (
                        $combined_datasetmodulemeta = array_merge_recursive($immutable_datasetmodulemeta ?? array(), $mutableonmodel_datasetmodulemeta ?? array(), $mutableonrequest_datasetmodulemeta ?? array())
                    ) {
        ...
        // Allow PoP UserState to add the lazy-loaded userstate data triggers
        HooksAPIFacade::getInstance()->doAction('\PoP\ComponentModel\Engine:getModuleData:end', $root_module, array(&$root_model_props), array(&$root_props), array(&$this->helperCalculations), $this);
    ...
    public function moveEntriesUnderDBName(array $entries, bool $entryHasId, $typeResolver): array
    {
        ...
        if ($entries) {
            ...
            $dbname_datafields = HooksAPIFacade::getInstance()->applyFilters('PoP\ComponentModel\Engine:moveEntriesUnderDBName:dbName-dataFields', [], $typeResolver);
            foreach ($dbname_datafields as $dbname => $data_fields) {
                ...
                if ($entryHasId) {
                    ...
                    $entry_data_fields_to_move = array_intersect(array_keys($dbname_entries['primary']), $data_fields);
                    foreach ($entry_data_fields_to_move as $data_field) {
    ...
    public function getDatabases()
    {
        ...
        while (!empty($this->typeResolverClass_ids_data_fields)) {
            ...
            if ($iterationDBItems) {
                ...
                foreach ($ids_data_fields as $id => $data_fields) {
                    foreach ($data_fields['conditional'] as $conditionDataField => $conditionalDataFields) {
                        $already_loaded_ids_data_fields[$typeResolver_class][(string)$id] = array_merge($already_loaded_ids_data_fields[$typeResolver_class][(string)$id] ?? [], array_intersect($conditionalDataFields, array_keys($iterationDBItems[(string)$id])));
                    }
                }
            ...
            if (!empty($iterationSchemaErrors) || !empty($storeSchemaErrors)) {
                ...
                foreach ($dbNameSchemaErrorEntries as $dbname => $entries) {
                    $schemaErrors[$dbname][$database_key] = array_merge($schemaErrors[$dbname][$database_key] ?? [], $entries);
                }
            ...
            if ($iterationSchemaWarnings) {
                ...
                foreach ($dbNameSchemaWarningEntries as $dbname => $entries) {
                    $schemaWarnings[$dbname][$database_key] = array_merge($schemaWarnings[$dbname][$database_key] ?? [], $entries);
                }
            }
            if ($iterationSchemaDeprecations) {
                ...
                foreach ($dbNameSchemaDeprecationEntries as $dbname => $entries) {
                    $schemaDeprecations[$dbname][$database_key] = array_merge($schemaDeprecations[$dbname][$database_key] ?? [], $entries);
                }
            }
            if ($iterationSchemaNotices) {
                ...
                foreach ($dbNameSchemaNoticeEntries as $dbname => $entries) {
                    $schemaNotices[$dbname][$database_key] = array_merge($schemaNotices[$dbname][$database_key] ?? [], $entries);
                }
            }
            if ($iterationSchemaTraces) {
                ...
                foreach ($dbNameSchemaTraceEntries as $dbname => $entries) {
                    $schemaTraces[$dbname][$database_key] = array_merge($schemaTraces[$dbname][$database_key] ?? [], $entries);
                }
            }
        ...
        HooksAPIFacade::getInstance()->doAction('\PoP\ComponentModel\Engine:traces:schema', $schemaTraces);
        HooksAPIFacade::getInstance()->doAction('\PoP\ComponentModel\Engine:traces:db', $dbTraces);
                    ...
                    foreach ($typeResolver_ids as $id) {
                        ...
                        foreach ($databases as $dbname => $database) {
                            if ($database_field_ids = $database[$database_key][(string)$id][$subcomponent_data_field_outputkey] ?? null) {
                                $subcomponentIDs[$dbname][$database_key][(string)$id] = array_merge($subcomponentIDs[$dbname][$database_key][(string)$id] ?? [], is_array($database_field_ids) ? $database_field_ids : array($database_field_ids));
                            }
                        }
                    }
                    ...
                    if ($field_ids) {
                        foreach ($field_ids as $field_id) {
                            ...
                            if ($subcomponent_already_loaded_data_fields = $subcomponent_already_loaded_ids_data_fields[$field_id] ?? null) {
                                $id_subcomponent_data_fields = array_values(array_diff($subcomponent_data_fields, $subcomponent_already_loaded_data_fields));
                                $id_subcomponent_conditional_data_fields = [];
                                foreach ($subcomponent_conditional_data_fields as $conditionField => $conditionalFields) {
                                    $id_subcomponent_conditional_data_fields[$conditionField] = array_values(array_diff($conditionalFields, $subcomponent_already_loaded_data_fields));
                                }
                            } else {
                        ...
                        $this->dbdata[$subcomponent_typeResolver_class][$module_path_key]['ids'] = array_merge($this->dbdata[$subcomponent_typeResolver_class][$module_path_key]['ids'] ?? [], $field_ids);
        ...
        if ($entries) {
            ...
            if ($dboutputmode == GD_URLPARAM_DATABASESOUTPUTMODE_SPLITBYDATABASES) {
                ...
                if ($entries = array_filter($entries)) {
                    $combined_databases = array();
                    foreach ($entries as $database_name => $database) {
                        $combined_databases = array_merge_recursive($combined_databases, $database);
                    }
                    $ret[$name] = $combined_databases;
                }
            }
    ...
    private function integrateSubcomponentDataProperties(&$dbdata, array $data_properties, $typeResolver_class, $module_path_key)
    {
        ...
        if ($subcomponents_data_properties = $data_properties['subcomponents'] ?? null) {
            ...
            $dbdata[$typeResolver_class][$module_path_key]['subcomponents'] = array_merge_recursive($dbdata[$typeResolver_class][$module_path_key]['subcomponents'] ?? array(), $subcomponents_data_properties);
        }
    }
}
