...
trait FieldOrDirectiveResolverTrait
{
    ...
    protected function maybeValidateNotMissingFieldOrDirectiveArguments(TypeResolverInterface $typeResolver, string $fieldOrDirectiveName, array $fieldOrDirectiveArgs, array $schemaFieldOrDirectiveArgs, string $type): ?string
    {
        if ($mandatoryArgs = SchemaHelpers::getSchemaMandatoryFieldArgs($schemaFieldOrDirectiveArgs)) {
            if (
                $maybeError = $this->validateNotMissingFieldOrDirectiveArguments(SchemaHelpers::getSchemaFieldArgNames($mandatoryArgs), $fieldOrDirectiveName, $fieldOrDirectiveArgs, $type)
            ) {
                return $maybeError;
            }
        }
    ...
    protected function validateNotMissingFieldOrDirectiveArguments(array $fieldOrDirectiveArgumentProperties, string $fieldOrDirectiveName, array $fieldOrDirectiveArgs, string $type): ?string
    {
        if ($missing = SchemaHelpers::getMissingFieldArgs($fieldOrDirectiveArgumentProperties, $fieldOrDirectiveArgs)) {
            ...
            return count($missing) == 1 ?
                sprintf($translationAPI->__('Argument \'%1$s\' cannot be empty, so %2$s \'%3$s\' has been ignored', 'component-model'), $missing[0], $type == ResolverTypes::FIELD ? $translationAPI->__('field', 'component-model') : $translationAPI->__('directive', 'component-model'), $fieldOrDirectiveName) :
                sprintf($translationAPI->__('Arguments \'%1$s\' cannot be empty, so %2$s \'%3$s\' has been ignored', 'component-model'), implode($translationAPI->__('\', \''), $missing), $type == ResolverTypes::FIELD ? $translationAPI->__('field', 'component-model') : $translationAPI->__('directive', 'component-model'), $fieldOrDirectiveName);
        }
        return null;
    }
        ...
        for ($i = 0; $i < count($fieldOrDirectiveArgumentNames); $i++) {
            ...
            if (!is_null($fieldOrDirectiveArgumentValue)) {
                ...
                if (is_null($fieldOrDirectiveArgumentValueDefinition)) {
                    ...
                    $deprecations[] = sprintf($translationAPI->__('Value \'%1$s\' for argument \'%2$s\' in %3$s \'%4$s\' is deprecated: \'%5$s\'', 'component-model'), $fieldOrDirectiveArgumentValue, $fieldOrDirectiveArgumentName, $type == ResolverTypes::FIELD ? $translationAPI->__('field', 'component-model') : $translationAPI->__('directive', 'component-model'), $fieldOrDirectiveName, $fieldOrDirectiveArgumentValueDefinition[SchemaDefinition::ARGNAME_DEPRECATIONDESCRIPTION]);
                }
            }
        }
        // if ($errors) {
