...
class GraphQLDataStructureFormatter extends MirrorQueryDataStructureFormatter
{
     ...
     * @param array<string,mixed> $data
     */
    protected function maybeAddTopLevelExtensionsEntryToResponse(&$ret, $data) : void
    {
     ...
     */
    protected function reformatGeneralEntries($entries) : array
    {
        $ret = [];
     ...
     */
    protected function getGeneralEntry($item) : array
    {
     ...
     * @param array<array<string,mixed>> $entries
     */
    protected function reformatDocumentEntries($entries) : array
    {
        $ret = [];
        foreach ($entries as $item) {
     ...
     * @param array<string,mixed> $item
     */
    protected function getDocumentEntry($item) : array
    {
        $entry = ['message' => $item[Tokens::MESSAGE]];
     ...
     * @param array<string,mixed> $item
     */
    protected function getDocumentEntryExtensions($item) : array
    {
        $extensions = [];
     ...
     * @return array<int,mixed[]>
     */
    protected function reformatSchemaEntries($entries) : array
    {
        $ret = [];
     ...
     * @return array<string,mixed>
     * @param array<string,mixed> $item
     * @param string $typeOutputKey
     */
    protected function getSchemaEntry($typeOutputKey, $item) : array
    {
        $entry = ['message' => $item[Tokens::MESSAGE]];
        if ($locations = $item[Tokens::LOCATIONS]) {
            $entry['locations'] = $locations;
     ...
     * @param string $typeOutputKey
     */
    protected function getSchemaEntryExtensions($typeOutputKey, $item) : array
    {
        $extensions = [];
        if ($path = $item[Tokens::PATH] ?? null) {
            $extensions['path'] = $path;
     ...
     * @param string $typeOutputKey
     */
    protected function getObjectEntry($typeOutputKey, $item) : array
    {
        $entry = ['message' => $item[Tokens::MESSAGE]];
        if ($locations = $item[Tokens::LOCATIONS]) {
            $entry['locations'] = $locations;
     ...
     * @param string $typeOutputKey
     */
    protected function getObjectEntryExtensions($typeOutputKey, $item) : array
    {
     ...
     * @param string $typeOutputKey
     */
    protected function validateObjectData($field, $typeOutputKey, &$sourceRet, &$resolvedObjectRet, $resolvedObject, $objectID) : bool
    {
