...
trait ComponentPathProcessorTrait
{
     ...
     */
    protected function getComponentProcessor($component) : \PoP\ComponentModel\ComponentProcessors\ComponentProcessorInterface
    {
     ...
     */
    protected function executeOnSelfAndPropagateToDatasetComponents($eval_self_fn, $propagate_fn, $component, &$props, $data_properties, $dataaccess_checkpoint_validation, $actionexecution_checkpoint_validation, $executed, $objectIDs) : array
    {
        $ret = [];
     ...
     * @param \PoP\Root\Feedback\FeedbackItemResolution|null $actionexecution_checkpoint_validation
     */
    protected function executeOnSelfAndMergeWithDatasetComponents($eval_self_fn, $propagate_fn, $component, &$props, $data_properties, $dataaccess_checkpoint_validation, $actionexecution_checkpoint_validation, $executed, $objectIDs) : array
    {
        $componentFullName = $this->getComponentHelpers()->getComponentFullName($component);
        // If componentPaths is provided, and we haven't reached the destination component yet, then do not execute the function at this level
        if (!$this->getComponentFilterManager()->excludeSubcomponent($component, $props)) {
     ...
     * @param \PoP\ComponentModel\Component\Component $component
     */
    protected function executeOnSelfAndPropagateToComponents($eval_self_fn, $propagate_fn, $component, &$props, $use_component_output_name_as_key = \true, $options = array()) : array
    {
        $ret = [];
        $componentFullName = $this->getComponentHelpers()->getComponentFullName($component);
     ...
     */
    protected function executeOnSelfAndMergeWithComponents($eval_self_fn, $propagate_fn, $component, &$props, $recursive = \true) : array
    {
        $componentFullName = $this->getComponentHelpers()->getComponentFullName($component);
        // If componentPaths is provided, and we haven't reached the destination component yet, then do not execute the function at this level
        if (!$this->getComponentFilterManager()->excludeSubcomponent($component, $props)) {
