...
class CustomPostUnionTypeHelpers
{
     ...
     * @param \PoP\ComponentModel\TypeResolvers\UnionType\UnionTypeResolverInterface|null $unionTypeResolver
     */
    public static function getCustomPostUnionOrTargetObjectTypeResolver($unionTypeResolver = null)
    {
        ...
        if ($targetTypeResolvers) {
            ...
            if ($moduleConfiguration->useSingleTypeInsteadOfCustomPostUnionType()) {
                return \count($targetTypeResolvers) === 1 ? $targetTypeResolvers[0] : $unionTypeResolver;
            }
        }
