...
class LoadCacheDirectiveResolver extends AbstractGlobalDirectiveResolver
{
    ...
    public function resolveDirective(TypeResolverInterface $typeResolver, array &$idsDataFields, array &$succeedingPipelineIDsDataFields, array &$succeedingPipelineDirectiveResolverInstances, array &$resultIDItems, array &$unionDBKeyIDs, array &$dbItems, array &$previousDBItems, array &$variables, array &$messages, array &$dbErrors, array &$dbWarnings, array &$dbDeprecations, array &$dbNotices, array &$dbTraces, array &$schemaErrors, array &$schemaWarnings, array &$schemaDeprecations, array &$schemaNotices, array &$schemaTraces) : void
    {
        ...
        if ($idsDataFieldsToRemove) {
            ...
            $saveCacheDirectiveResolver = $this->instanceManager->getInstance(\PoP\Engine\ConditionalOnContext\UseComponentModelCache\SchemaServices\DirectiveResolvers\SaveCacheDirectiveResolver::class);
            ...
            foreach ($idsDataFieldsToRemove as $id => $dataFieldsToRemove) {
                $logCachedIDFields[] = \sprintf($this->translationAPI->__('ID: %s, Field(s): \'%s\'', 'engine'), $id, \implode('\', \'', $dataFieldsToRemove['direct']));
            }
            $this->feedbackMessageStore->addLogEntry(\sprintf($this->translationAPI->__('The following fields of type \'%s\' were resolved from the cache - %s', 'engine'), $typeResolver->getTypeName(), \implode($this->translationAPI->__('; ', 'engine'), $logCachedIDFields)));
        }
    }
