        ...
        if (isset($directiveArgs['expressions'])) {
            ...
            if ($expressionsNameCount > $propertiesCount) {
                $schemaWarnings[] = [
                    ...
                    Tokens::MESSAGE => sprintf($translationAPI->__('Argument \'expressions\' has more elements than argument \'properties\', so the following expressions have been ignored: \'%s\'', 'component-model'), implode($translationAPI->__('\', \''), array_slice($expressionsName, $propertiesCount))),
                ];
            } elseif ($expressionsNameCount < $propertiesCount) {
                $schemaWarnings[] = [
                    Tokens::PATH => [$this->directive],
                    Tokens::MESSAGE => sprintf($translationAPI->__('Argument \'properties\' has more elements than argument \'expressions\', so the following properties will be assigned to the destination object under their same name: \'%s\'', 'component-model'), implode($translationAPI->__('\', \''), array_slice($properties, $expressionsNameCount))),
                ];
            }
        }
        ...
        foreach (array_keys($idsDataFields) as $id) {
            for ($i = 0; $i < count($properties); $i++) {
                ...
                if (!$isValueInDBItems && !array_key_exists($property, $previousDBItems[$dbKey][(string)$id] ?? [])) {
                    $dbErrors[(string)$id][] = [
                        Tokens::PATH => [$this->directive],
                        Tokens::MESSAGE => sprintf($translationAPI->__('Property \'%s\' hadn\'t been set for object with ID \'%s\', so no expression has been defined', 'component-model'), $property, $id),
                    ];
                    continue;
                }
                ...
                if (!is_null($existingValue)) {
                    $dbWarnings[(string)$id][] = [
                        Tokens::PATH => [$this->directive],
                        Tokens::MESSAGE => sprintf($translationAPI->__('The existing value for expression \'%s\' for object with ID \'%s\' has been overriden: \'%s\'', 'component-model'), $expressionName, $id),
                    ];
                }
