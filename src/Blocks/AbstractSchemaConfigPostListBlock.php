        ...
        if ($postListIDs = $attributes[$this->getAttributeName()] ?? []) {
            ...
            foreach ($postObjects as $postObject) {
                ...
                $postContentElems[] = ($permalink ?
                    \sprintf('<code><a href="%s">%s</a></code>', $permalink, BlockRenderingHelpers::getCustomPostTitle($postObject)) :
                    \sprintf('<code>%s</code>', BlockRenderingHelpers::getCustomPostTitle($postObject))
                ) . ($postDescription ?
            ...
            $notFoundPostListIDs = array_diff($postListIDs, $foundPostListIDs);
            foreach ($notFoundPostListIDs as $notFoundPostID) {
                $postContentElems[] = \sprintf('<code>%s</code>', \sprintf(\__('Undefined item with ID %s', 'graphql-api'), $notFoundPostID));
            }
        }
        $className = $this->getBlockClassName();
        return sprintf($blockContentPlaceholder, $className, $className . '-front', $this->getHeader(), $postContentElems ?
            sprintf('<ul><li>%s</li></ul>', implode('</li><li>', $postContentElems))
            : sprintf('<em>%s</em>', \__('(not set)', 'graphql-api')));
    }
    /**
     * Register index.css
     *
