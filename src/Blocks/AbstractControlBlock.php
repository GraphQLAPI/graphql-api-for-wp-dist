    ...
    public function renderBlock(array $attributes, string $content): string
    {
            ...
            if ($typeFields) {
                ...
                if ($groupFieldsUnderTypeForPrint) {
                    ...
                    foreach ($typeFieldsForPrint as $typeName => $fields) {
                        $fieldTypeContent .= sprintf('<strong>%s</strong><ul><li><code>%s</code></li></ul>', $typeName, implode('</code></li><li><code>', $fields));
                    }
                } else {
                    /**
                     * Cast object so PHPStan doesn't throw error
                     ...
                     */
                    $typeFieldsForPrint = $typeFieldsForPrint;
                    $fieldTypeContent = sprintf('<ul><li>%s</li></ul>', implode('</li><li>', $typeFieldsForPrint));
                }
            }
        }
        ...
        if (!$this->disableFields() && !$this->disableDirectives()) {
            $blockDataPlaceholder = <<<EOT
                ...
                %s
EOT;
            $blockDataContent = sprintf($blockDataPlaceholder, __('Fields', 'graphql-api'), $fieldTypeContent, __('Directives', 'graphql-api'), $directiveContent);
...
EOT;
        return sprintf($blockContentPlaceholder, $className . ' ' . $this->getAlignClass(), $className . '__data', $className . '__title', $this->getBlockDataTitle(), $blockDataContent, $className . '__content', $className . '__title', $this->getBlockContentTitle(), $this->getBlockContent($attributes, $content));
    }
