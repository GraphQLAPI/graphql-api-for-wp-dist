...
class GraphQLEndpointPostType extends AbstractGraphQLQueryExecutionPostType
{
    ...
    protected function getPostTypeLabels(string $name_uc, string $names_uc, string $names_lc): array
    {
         ...
         */
        return array_merge(parent::getPostTypeLabels($name_uc, $names_uc, $names_lc), array(
    ...
    {
        return !in_array($_REQUEST[RequestParams::VIEW] ?? null, [
    ...
    protected function doSomethingElse(): void
    {
        if (($_REQUEST[RequestParams::VIEW] ?? null) == RequestParams::VIEW_SOURCE) {
             ...
             */
            \add_action('wp', [$this, 'maybePrintClient'], PHP_INT_MAX);
        }
    }
    /**
    ...
    public function maybePrintClient(): void
    {
        ...
        if (
            ...
            $useGraphiQLExplorer = $moduleRegistry->isModuleEnabled(ClientFunctionalityModuleResolver::GRAPHIQL_EXPLORER) && $userSettingsManager->getSetting(ClientFunctionalityModuleResolver::GRAPHIQL_EXPLORER, ClientFunctionalityModuleResolver::OPTION_USE_IN_PUBLIC_CLIENT_FOR_CUSTOM_ENDPOINTS);
    ...
    protected function isGraphiQLEnabled($postOrID): bool
    {
        ...
        return $this->isOptionsBlockValueOn($postOrID, EndpointOptionsBlock::ATTRIBUTE_NAME_IS_GRAPHIQL_ENABLED, true);
    }
    ...
    protected function isVoyagerEnabled($postOrID): bool
    {
        ...
        // `true` is the default option in Gutenberg, so it's not saved to the DB!
        return $this->isOptionsBlockValueOn($postOrID, EndpointOptionsBlock::ATTRIBUTE_NAME_IS_VOYAGER_ENABLED, true);
    }
        ...
        return array_merge(
            ...
            $isGraphiQLEnabled ? [
                'graphiql' => sprintf(
                    ...
                    \add_query_arg(RequestParams::VIEW, RequestParams::VIEW_GRAPHIQL, $permalink),
            ...
            $isVoyagerEnabled ? [
                'schema' => sprintf(
                    '<a href="%s" rel="bookmark" aria-label="%s">%s</a>',
                    \add_query_arg(RequestParams::VIEW, RequestParams::VIEW_SCHEMA, $permalink),
                    /* translators: %s: Post title. */
