...
trait ModulePathProcessorTrait
{
    ...
    {
        $ret = array();
        $key = \PoP\ComponentModel\Modules\ModuleUtils::getModuleOutputName($module);
        ...
        foreach ($submodules as $submodule) {
            $submodules_ret = \array_merge($submodules_ret, $this->getModuleProcessor($submodule)->{$propagate_fn}($submodule, $props[$moduleFullName][\PoP\ComponentModel\Constants\Props::SUBMODULES], $data_properties, $dataaccess_checkpoint_validation, $actionexecution_checkpoint_validation, $executed, $dbobjectids));
        }
        if ($submodules_ret) {
    ...
    protected function executeOnSelfAndMergeWithDatasetmodules($eval_self_fn, $propagate_fn, array $module, array &$props, array $data_properties, $dataaccess_checkpoint_validation, $actionexecution_checkpoint_validation, $executed, $dbobjectids)
    {
        ...
        $modulefilter_manager->prepareForPropagation($module, $props);
        foreach ($submodules as $submodule) {
            $ret = \array_merge_recursive($ret, $this->getModuleProcessor($submodule)->{$propagate_fn}($submodule, $props[$moduleFullName][\PoP\ComponentModel\Constants\Props::SUBMODULES], $data_properties, $dataaccess_checkpoint_validation, $actionexecution_checkpoint_validation, $executed, $dbobjectids));
        }
        $modulefilter_manager->restoreFromPropagation($module, $props);
        return $ret;
    }
    ...
    protected function executeOnSelfAndPropagateToModules($eval_self_fn, $propagate_fn, array $module, array &$props, $use_module_output_name_as_key = \true, $options = array())
    {
        $ret = array();
        $moduleFullName = \PoP\ComponentModel\Modules\ModuleUtils::getModuleFullName($module);
        $key = $use_module_output_name_as_key ? \PoP\ComponentModel\Modules\ModuleUtils::getModuleOutputName($module) : $moduleFullName;
        // If modulepaths is provided, and we haven't reached the destination module yet, then do not execute the function at this level
        $modulefilter_manager = \PoP\ComponentModel\Facades\ModuleFiltering\ModuleFilterManagerFacade::getInstance();
        if (!$modulefilter_manager->excludeModule($module, $props)) {
        ...
        $submodules_ret = array();
        foreach ($submodules as $submodule) {
            $submodules_ret = \array_merge($submodules_ret, $this->getModuleProcessor($submodule)->{$propagate_fn}($submodule, $props[$moduleFullName][\PoP\ComponentModel\Constants\Props::SUBMODULES]));
        }
        if ($submodules_ret) {
            $ret[$key][POP_RESPONSE_PROP_SUBMODULES] = $submodules_ret;
        }
    ...
    protected function executeOnSelfAndMergeWithModules($eval_self_fn, $propagate_fn, array $module, array &$props, $recursive = \true)
    {
        $moduleFullName = \PoP\ComponentModel\Modules\ModuleUtils::getModuleFullName($module);
        // If modulepaths is provided, and we haven't reached the destination module yet, then do not execute the function at this level
        $modulefilter_manager = \PoP\ComponentModel\Facades\ModuleFiltering\ModuleFilterManagerFacade::getInstance();
        ...
        foreach ($submodules as $submodule) {
            ...
            $ret = $recursive ? \array_merge_recursive($ret, $submodule_ret) : \array_unique(\array_values(\array_merge($ret, $submodule_ret)));
        }
        $modulefilter_manager->restoreFromPropagation($module, $props);
        return $ret;
    }
