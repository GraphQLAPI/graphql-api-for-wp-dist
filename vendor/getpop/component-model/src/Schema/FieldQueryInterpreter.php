...
class FieldQueryInterpreter extends \PoP\FieldQuery\FieldQueryInterpreter implements FieldQueryInterpreterInterface
{
            ...
            if ($separatorPos === false) {
                ...
                if (!$orderedFieldOrDirectiveArgNamesEnabled || !isset($orderedFieldOrDirectiveArgNames[$i])) {
                    ...
                    $schemaWarnings[] = [
                        ...
                        Tokens::MESSAGE => sprintf($this->translationAPI->__('The argument on position number %s (with value \'%s\') has its name missing, and %s. Please define the query using the \'key%svalue\' format. This argument has been ignored', 'pop-component-model'), $i + 1, $fieldOrDirectiveArgValue, $errorMessage, QuerySyntax::SYMBOL_FIELDARGS_ARGKEYVALUESEPARATOR),
                    ];
                ...
                if (!array_key_exists($fieldOrDirectiveArgName, $fieldOrDirectiveArgumentNameTypes)) {
                    $schemaWarnings[] = [
                        Tokens::PATH => [$fieldOrDirective],
                        Tokens::MESSAGE => sprintf($this->translationAPI->__('Argument with name \'%s\' has not been documented in the schema, so it may have no effect (it has not been removed from the query, though)', 'pop-component-model'), $fieldOrDirectiveArgName),
                    ];
                }
            }
    ...
    protected function extractFieldOrDirectiveArgumentsForResultItem(
        ...
        // Only need to extract arguments if they have fields or arrays
        if (
            FieldQueryUtils::isAnyFieldArgumentValueDynamic(array_values($fieldOrDirectiveArgs))
        ) {
            $id = $typeResolver->getID($resultItem);
        ...
        if (
            ...
            foreach (array_keys($failedCastingFieldArgs) as $failedCastingFieldArgName) {
                ...
                if ($fieldArgErrorMessage = $failedCastingFieldArgErrorMessages[$failedCastingFieldArgName] ?? null) {
                    $errorMessage = sprintf($this->translationAPI->__('For field \'%s\', casting value \'%s\' for argument \'%s\' to type \'%s\' failed: %s. It has been ignored', 'pop-component-model'), $fieldName, is_array($fieldArgs[$failedCastingFieldArgName]) ? json_encode($fieldArgs[$failedCastingFieldArgName]) : $fieldArgs[$failedCastingFieldArgName], $failedCastingFieldArgName, TypeCastingHelpers::getTypeCombinationCurrentElement($fieldArgNameTypes[$failedCastingFieldArgName]), $fieldArgErrorMessage);
                } else {
                    $errorMessage = sprintf($this->translationAPI->__('For field \'%s\', casting value \'%s\' for argument \'%s\' to type \'%s\' failed, so it has been ignored', 'pop-component-model'), $fieldName, is_array($fieldArgs[$failedCastingFieldArgName]) ? json_encode($fieldArgs[$failedCastingFieldArgName]) : $fieldArgs[$failedCastingFieldArgName], $failedCastingFieldArgName, TypeCastingHelpers::getTypeCombinationCurrentElement($fieldArgNameTypes[$failedCastingFieldArgName]));
                }
                $schemaWarnings[] = [
                    Tokens::PATH => [$field],
                    Tokens::MESSAGE => $errorMessage,
