...
class FilterMultipleInputModuleProcessor extends AbstractFormInputModuleProcessor implements DataloadQueryArgsFilterInputModuleProcessorInterface, DataloadQueryArgsSchemaFilterInputModuleProcessorInterface
{
    ...
    {
        return array([self::class, self::MODULE_FILTERINPUT_CUSTOMPOSTDATES]);
    }
    public function getFilterInput(array $module) : ?array
    {
        $filterInputs = [self::MODULE_FILTERINPUT_CUSTOMPOSTDATES => [FilterInputProcessor::class, FilterInputProcessor::FILTERINPUT_CUSTOMPOSTDATES]];
    ...
    public function getName(array $module)
    {
        switch ($module[1]) {
            case self::MODULE_FILTERINPUT_CUSTOMPOSTDATES:
                ...
                $names = array(self::MODULE_FILTERINPUT_CUSTOMPOSTDATES => 'date');
    ...
    public function getSchemaFilterInputDescription(array $module) : ?string
    {
        ...
        switch ($module[1]) {
            case self::MODULE_FILTERINPUT_CUSTOMPOSTDATES:
                ...
                return \sprintf($this->translationAPI->__('Search for posts between the \'from\' and \'to\' dates. Provide dates through params \'%s\' and \'%s\'', 'pop-posts'), $formInputHelperService->getMultipleInputName($name, $subnames[0]), $formInputHelperService->getMultipleInputName($name, $subnames[1]));
        }
        return null;
    }
}
