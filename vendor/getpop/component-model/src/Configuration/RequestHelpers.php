...
class RequestHelpers
{
     ...
     * @param string $url
     */
    public static function maybeAddParamToDebugRequest($url) : string
    {
        if (RootEnvironment::isApplicationEnvironmentDev() && App::getRequest()->query->has('XDEBUG_TRIGGER')) {
            $url = GeneralUtils::addQueryArgs([FrameworkParams::XDEBUG_TRIGGER => (string) App::getRequest()->query->get('XDEBUG_TRIGGER')], $url);
        }
        return $url;
    }
}
