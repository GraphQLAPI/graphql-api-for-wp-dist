...
class CommonFilterMultipleInputModuleProcessor extends AbstractFormInputModuleProcessor implements DataloadQueryArgsFilterInputModuleProcessorInterface, DataloadQueryArgsSchemaFilterInputModuleProcessorInterface
{
    ...
    {
        return array([self::class, self::MODULE_FILTERINPUT_DATES]);
    }
    public function getFilterInput(array $module) : ?array
    {
        $filterInputs = [self::MODULE_FILTERINPUT_DATES => [FilterInputProcessor::class, FilterInputProcessor::FILTERINPUT_DATES]];
    ...
    public function getName(array $module)
    {
        ...
        $names = array(self::MODULE_FILTERINPUT_DATES => 'date');
    ...
    protected function modifyFilterSchemaDefinitionItems(array &$schemaDefinitionItems, array $module) : void
    {
        ...
        switch ($module[1]) {
            case self::MODULE_FILTERINPUT_DATES:
                ...
                $schemaDefinitionItems[] = \array_merge([SchemaDefinition::ARGNAME_NAME => $formInputHelperService->getMultipleInputName($name, $subnames[0])], $schemaDefinition, [SchemaDefinition::ARGNAME_DESCRIPTION => \sprintf($this->translationAPI->__('Search for elements starting from this date, in format \'%s\'', 'pop-engine'), $dateFormat)]);
                $schemaDefinitionItems[] = \array_merge([SchemaDefinition::ARGNAME_NAME => $formInputHelperService->getMultipleInputName($name, $subnames[1])], $schemaDefinition, [SchemaDefinition::ARGNAME_DESCRIPTION => \sprintf($this->translationAPI->__('Search for elements starting until this date, in format \'%s\'', 'pop-engine'), $dateFormat)]);
                break;
        }
    }
    public function getSchemaFilterInputType(array $module) : string
    {
        switch ($module[1]) {
            case self::MODULE_FILTERINPUT_DATES:
                return SchemaDefinition::TYPE_DATE;
            default:
    ...
    public function getSchemaFilterInputDescription(array $module) : ?string
    {
        ...
        switch ($module[1]) {
            case self::MODULE_FILTERINPUT_DATES:
                $name = $this->getName($module);
                $subnames = $this->getInputOptions($module)['subnames'];
                return \sprintf($this->translationAPI->__('Search for elements between the \'from\' and \'to\' dates. Provide dates through params \'%s\' and \'%s\', in format \'%s\'', 'pop-engine'), $formInputHelperService->getMultipleInputName($name, $subnames[0]), $formInputHelperService->getMultipleInputName($name, $subnames[1]), $this->cmsService->getOption($this->nameResolver->getName('popcms:option:dateFormat')));
        }
        return null;
    }
}
