...
final class ResolveValueAndMergeDirectiveResolver extends \PoP\ComponentModel\DirectiveResolvers\AbstractGlobalDirectiveResolver implements MandatoryDirectiveServiceTagInterface
{
    ...
    protected function resolveValueForResultItems(TypeResolverInterface $typeResolver, array &$resultIDItems, array &$idsDataFields, array &$dbItems, array &$previousDBItems, array &$variables, array &$messages, array &$dbErrors, array &$dbWarnings, array &$dbDeprecations, array &$schemaErrors, array &$schemaWarnings, array &$schemaDeprecations)
    {
        ...
        foreach (\array_keys($idsDataFields) as $id) {
            ...
            if (\is_null($resultItem)) {
                $dbErrors[(string) $id][] = [Tokens::PATH => ['id'], Tokens::MESSAGE => \sprintf($this->translationAPI->__('Corrupted data: Object with ID \'%s\' doesn\'t exist', 'component-model'), $id)];
    ...
    {
        $errorOutput = [Tokens::MESSAGE => $error->getMessageOrCode(), Tokens::EXTENSIONS => $error->getData()];
    ...
    protected function addValueForResultItem(TypeResolverInterface $typeResolver, $id, string $field, $value, array &$dbItems, array &$dbErrors)
    {
        ...
        if (GeneralUtils::isError($value)) {
            ...
            /** @var Error */
            $error = $value;
            $dbErrors[(string) $id][] = \array_merge([Tokens::PATH => [$field]], $this->getErrorOutput($error));
